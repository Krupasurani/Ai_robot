...
function MotivationIllustration({ hideBackground, ...other }: Props) {
  const { theme } = useTheme();

  const PRIMARY_LIGHTER = theme === 'dark' ? '#bae6fd' : '#e0f2fe';
  const PRIMARY_DARK = theme === 'dark' ? '#0369a1' : '#1d4ed8';
  const PRIMARY_DARKER = theme === 'dark' ? '#075985' : '#1e40af';
  ...
  return (
    <svg
      ...
      className="h-auto w-[320px] max-w-full flex-shrink-0"
      {...other}
    >
