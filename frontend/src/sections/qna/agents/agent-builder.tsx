...
const AgentBuilder: React.FC<AgentBuilderProps> = ({ editingAgent, onSuccess, onClose }) => {
    ...
    if (!loading && availableModels.length > 0 && nodes.length === 0) {
      ...
      const initialNodes = [
        ...
        },
        {
          ...
          data: {
            id: 'llm-1',
            type: `llm-${initalModel?.provider || 'azureOpenAI'}-${initalModel?.modelName || 'default'}`
              .replace(/[^a-zA-Z0-9]/g, '-')
              .toLowerCase(),
            label:
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] flex-col overflow-hidden rounded-2xl border border-border/60 bg-card shadow-sm">
      {/* Header */}
      <AgentBuilderHeader
