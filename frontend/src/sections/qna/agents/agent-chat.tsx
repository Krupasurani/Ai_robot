...
import { formatStepLabel } from '../chatbot/utils/useChatBot';
...
const getEngagingStatusMessage = (event: string, data: any): string | null => {
  ...
  switch (event) {
    ...
    case 'thinking_complete':
      return data?.thinking ? `🧠 ${data.thinking}` : baseMessage || '🧠 Reasoning complete.';
    case 'query_decomposed': {
      const queryCount = data.queries?.length || 0;
      return queryCount > 1
        ? `🧩 Breaking your request into ${queryCount} intents for better coverage.`
        : '🤔 Analyzing your request...';
    }
    case 'search_complete': {
      const resultsCount = data.results_count || 0;
      return resultsCount > 0
        ? `📚 Found ${resultsCount} potential sources. Processing them now...`
        : '✅ Finished searching...';
    }
    case 'connected':
      return '🔌 Connected and processing...';
    default:
      return baseMessage || 'Processing...';
  }
};
  ...
  const currentStreamingState = useMemo(() => {
    ...
    return state
      ? {
          ...
          thinking: '', // Not yet supported in agent chat state
          toolCalls: [], // Not yet supported in agent chat state
          webSources: [], // Not yet supported in agent chat state
          isActive: state.isActive,
        }
      : {
          messageId: null,
  ...
  const handleStreamingResponse = useCallback(
    async (url: string, body: any, isNewConversation: boolean): Promise<string | null> => {
      ...
      const handleStreamingEvent = async (
        ...
        switch (event) {
          case 'meta':
            // Currently unused in Agent UI; reserved for future knowledge indicator
            break;
          case 'answer_chunk':
            if (data.chunk) {
  ...
  return (
    <StreamingContext.Provider value={streamingContextValue}>
      <div className="flex w-full h-[90vh] overflow-hidden">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                size="icon"
                variant="outline"
                className="absolute top-[78px] left-[14px] z-[1100] h-9 w-9"
        ...
        {isDrawerOpen && (
          <div className="w-[300px] border-r border-border bg-background overflow-hidden flex-shrink-0">
            <AgentChatSidebar
        ...
        >
          <div
            className={cn(
              'flex flex-col min-w-0 h-[90vh] relative',
          ...
          {/* PDF Viewer */}
          {openPdfView && (
            <div className="h-[90vh] overflow-hidden relative bg-background [&>div]:h-full [&>div]:w-full">
              {transitioning && (
