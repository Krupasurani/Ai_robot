...
// Markdown content styles - injected via useEffect
const createMarkdownStyles = (): (() => void) => {
  const styleId = 'markdown-viewer-styles';
  if (document.getElementById(styleId)) {
    return () => {}; // Return no-op cleanup if already exists
  }
  ...
  style.textContent = `
    ...
    .dark .markdown-document-container {
      background-color: transparent;
    }

    /* Text formatting */
    .markdown-document-container p,
    .markdown-document-container li,
    .markdown-document-container blockquote {
      line-height: 1.6;
      color: hsl(var(--foreground));
    }

    .dark .markdown-document-container p,
    .dark .markdown-document-container li,
    .dark .markdown-document-container blockquote {
      color: hsl(var(--foreground) / 0.9);
    }

    /* Headings */
    .markdown-document-container h1,
    .markdown-document-container h2,
    .markdown-document-container h3,
    .markdown-document-container h4,
    .markdown-document-container h5,
    .markdown-document-container h6 {
      margin-top: 1.5em;
      margin-bottom: 0.8em;
      color: hsl(var(--foreground));
    }

    .dark .markdown-document-container h1,
    .dark .markdown-document-container h2,
    .dark .markdown-document-container h3,
    .dark .markdown-document-container h4,
    .dark .markdown-document-container h5,
    .dark .markdown-document-container h6 {
      color: hsl(var(--primary) / 0.9);
    }

    /* Tables */
    .markdown-document-container table {
    ...
    .dark .markdown-document-container table {
      border-color: hsl(var(--border) / 0.1);
    }

    .markdown-document-container th,
    .markdown-document-container td {
      border: 1px solid #ddd;
      padding: 0.5em;
      text-align: left;
    }

    .dark .markdown-document-container th,
    .dark .markdown-document-container td {
      border-color: hsl(var(--border) / 0.1);
    }
    ...
    /* Text formatting */
    .markdown-document-container strong,
    .markdown-document-container b {
      font-weight: bold;
      color: inherit;
    }

    .dark .markdown-document-container strong,
    .dark .markdown-document-container b {
      color: hsl(var(--primary) / 0.8);
    }

    .markdown-document-container em,
    .markdown-document-container i {
      font-style: italic;
    }

    /* Code blocks */
    .markdown-document-container code {
    ...
    .dark .markdown-document-container code {
      background-color: hsl(var(--muted) / 0.6);
      color: hsl(var(--primary) / 0.9);
    }

    .markdown-document-container pre {
    ...
    .dark .markdown-document-container pre {
      background-color: hsl(var(--muted) / 0.6);
      border-color: hsl(var(--border) / 0.2);
    }

    .markdown-document-container pre > code {
      display: block;
      padding: 1em;
      overflow-x: auto;
      background-color: transparent;
    ...
    .dark .markdown-document-container pre > code {
      color: hsl(var(--foreground) / 0.85);
    }

    /* Syntax highlighting enhancement for dark mode */
    .dark .markdown-document-container pre .token.comment {
      color: #6a9955;
    }
    .dark .markdown-document-container pre .token.string {
      color: #ce9178;
    }
    .dark .markdown-document-container pre .token.keyword {
      color: #569cd6;
    }
    .dark .markdown-document-container pre .token.function {
      color: #dcdcaa;
    }
    .dark .markdown-document-container pre .token.number {
      color: #b5cea8;
    }
    ...
    /* Links */
    .markdown-document-container a {
      color: hsl(var(--primary));
      text-decoration: none;
    }

    .markdown-document-container a:hover {
      text-decoration: underline;
      color: hsl(var(--primary) / 0.8);
    }

    .dark .markdown-document-container a:hover {
      color: hsl(var(--primary) / 0.9);
    }
  `;
...
const MarkdownViewer: React.FC<MarkdownViewerProps> = ({
  ...
  // Create markdown styles on mount
  useEffect(() => {
    const cleanup = createMarkdownStyles();
    return cleanup;
  }, []);
  ...
  return (
    <div
      ...
      className={cn(
        'w-full h-full relative overflow-hidden rounded-md border border-border',
        sx && Object.keys(sx).length > 0 ? '' : ''
      )}
      style={sx as React.CSSProperties}
    >
      {loading && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/80 z-10">
          <Loader2 size={40} className="mb-4 animate-spin text-muted-foreground" />
          <p className="text-base text-foreground">Loading Document...</p>
        </div>
      )}
      ...
      {error && !loading && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-destructive/10 text-destructive p-4 text-center z-10">
          ...
          <h6 className="text-lg font-semibold mb-2 text-destructive">Loading Error</h6>
          <p className="text-base text-destructive/80">{error}</p>
        </div>
      )}
      ...
      <div className={cn('flex h-full w-full', loading || error ? 'invisible' : 'visible')}>
        ...
        <div
          className={cn(
            'h-full flex-grow relative transition-all duration-300 ease-in-out',
            processedCitations.length > 0 ? 'w-[calc(100%-280px)] border-r border-border' : 'w-full'
          )}
        >
          <div ref={containerRef} className={cn('markdown-document-container')}>
            ...
            {!loading && !error && !markdownContent && documentReady && (
              <p className="p-6 text-muted-foreground text-center mt-8">
        ...
        {processedCitations.length > 0 && !loading && !error && (
          <div className="w-[280px] h-full flex-shrink-0 overflow-y-auto">
            <CitationSidebar
