...
interface UseConnectorManagerReturn {
  ...
  handleDeleteConnector: () => Promise<void>;
...
export const useConnectorManager = (): UseConnectorManagerReturn => {
  ...
  const handleDeleteConnector = useCallback(async () => {
    ...
    try {
      ...
      // Step 1: Remove any crawler schedules
      try {
        await CrawlingManagerApi.remove(connector.name.toLowerCase());
      } catch (scheduleError) {
        // Ignore errors here - schedule might not exist
        console.warn('Could not remove crawler schedule:', scheduleError);
      }

      // Step 2: Delete the connector and all its data
      const response = await ConnectorApiService.deleteConnector(connector.name);

      if (response.success) {
        setSuccessMessage(`${connector.name} deleted successfully`);
        setSuccess(true);
