...
export const useConnectorConfig = ({
  ...
  const mergeConfigWithSchema = useCallback(
    (configResponse: any, schemaResponse: any) => {
      // Ensure schema response has required structures with defaults
      const authSchema = schemaResponse?.auth || { schema: { fields: [] }, customFields: [] };
      const syncSchema = schemaResponse?.sync || { supportedStrategies: ['MANUAL'], customFields: [] };
      const filtersSchema = schemaResponse?.filters || { schema: { fields: [] }, customFields: [] };
      ...
      return {
        ...
        config: {
          ...
          auth: {
            ...authSchema,
            schema: authSchema.schema || { fields: [] },
            customFields: authSchema.customFields || [],
            values: configResponse.config?.auth?.values || configResponse.config?.auth || {},
            customValues: configResponse.config?.auth?.customValues || {},
          },
          sync: {
            ...syncSchema,
            supportedStrategies: syncSchema.supportedStrategies || ['MANUAL'],
            customFields: syncSchema.customFields || [],
            selectedStrategy:
              configResponse.config?.sync?.selectedStrategy ||
              syncSchema.supportedStrategies?.[0] ||
              'MANUAL',
          ...
          filters: {
            ...filtersSchema,
            schema: filtersSchema.schema || { fields: [] },
            customFields: filtersSchema.customFields || [],
  ...
  useEffect(() => {
    const fetchConnectorConfig = async () => {
      // Guard against missing connector
      if (!connector?.name) {
        setLoading(false);
        return;
      }
