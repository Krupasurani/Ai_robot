...
export class ConnectorApiService {
    ...
    static async deleteConnector(connectorName: string): Promise<{ success: boolean; message: string; statistics: any }> {
        const response = await axios.delete(`${BASE_URL}/${connectorName}`);
        if (!response.data) throw new Error('Failed to delete connector');
        return response.data;
    }
