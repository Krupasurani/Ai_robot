...
export const getUserLogo = async (_userId: string): Promise<string | null> => {
  try {
    // Backend expects current user context, no :id in path
    const response = await axios.get(`${CONFIG.backendUrl}/api/v1/users/dp`, {
      responseType: 'arraybuffer',
      // Treat missing image responses as non-errors to avoid console noise
...
export const getUserLogoById = async (userId: string): Promise<string | null> => {
  try {
...
export const updateUser = async (userId: string, userData: any) => {
  try {
    ...
    // Axios interceptor transforms errors into ProcessedError: { type, message, code, statusCode, details }
    ...
    if (status === 404 || errorCode === 'NOT_FOUND') {
      throw new Error('User not found. The user may have been deleted or does not exist.');
    }
...
export const uploadUserLogo = async (_userId: string, formData: any) => {
  try {
    // Backend expects current user context, no :id in path
    const response = await axios.put(`${CONFIG.backendUrl}/api/v1/users/dp`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
...
export const deleteUserLogo = async (_userId: string) => {
  try {
    // Backend expects current user context, no :id in path
    const response = await axios.delete(`${CONFIG.backendUrl}/api/v1/users/dp`);
    return response.data;
  } catch (error) {
