...
/**
 * Fetch Entra ID authentication configuration
 * @returns {Promise<AzureAuthConfig>} The Entra ID auth configuration
 */
export const getAzureAuthConfig = async (): Promise<AzureAuthConfig> => {
  try {
    ...
    console.error('Failed to fetch Entra ID auth configuration:', error);
    throw error;
  }
};
...
/**
 * Update Entra ID auth configuration
 * @param {AzureAuthConfig} azureConfig - Entra ID auth configuration
 * @returns {Promise<any>} The API response
 */
export const updateAzureAuthConfig = async (azureConfig: AzureAuthConfig): Promise<any> => {
  try {
    ...
    console.error('Failed to update Entra ID auth configuration:', error);
    throw error;
  }
};
