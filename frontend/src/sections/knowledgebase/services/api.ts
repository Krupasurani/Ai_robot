...
export class KnowledgeBaseAPI {
  ...
  // Transform ArangoDB record group to frontend KnowledgeBase format
  private static transformKnowledgeBase(kb: any): any {
    return {
      ...kb,
      // Map ArangoDB field names to frontend expected names
      name: kb.name || kb.groupName,
      ownerId: kb.ownerId || kb.createdBy,
  ...
  static async getKnowledgeBases(params?: {
    ...
    if (response.data.knowledgeBases && response.data.pagination) {
      // Paginated response - transform each knowledge base
      return {
        knowledgeBases: response.data.knowledgeBases.map(this.transformKnowledgeBase),
        pagination: response.data.pagination,
      };
    }
    if (Array.isArray(response.data)) {
      // Simple array response (fallback) - transform each item
      return response.data.map(this.transformKnowledgeBase);
    }
    // Handle other response formats
    return response.data;
  }

  static async createKnowledgeBase(name: string, icon?: string, description?: string): Promise<KnowledgeBase> {
    const response = await axios.post(`${API_BASE}/`, { kbName: name, icon, description });
  ...
  static async getKnowledgeBase(kbId: string): Promise<KnowledgeBase> {
    ...
    return this.transformKnowledgeBase(response.data);
  }

  static async updateKnowledgeBase(kbId: string, name: string, icon?: string): Promise<KnowledgeBase> {
    const response = await axios.put(`${API_BASE}/${kbId}`, { groupName: name, icon });
  ...
  static async getKnowledgeBaseRecordCount(kbId: string): Promise<number | null> {
    const response = await axios.get(`${API_BASE}/${kbId}/records`, {
      params: { page: 1, limit: 1 },
    });
    if (!response.data) throw new Error('Failed to fetch knowledge base record count');

    const data = response.data as any;

    const candidates = [
      data?.counts?.totalRecords,
      data?.counts?.records,
      data?.counts?.docs,
    ...
    const count = candidates.find((v) => typeof v === 'number' && Number.isFinite(v));
  ...
  private static async fetchRecordDetailsFromConnector(
    recordId: string
  ): Promise<RecordDetailsResponse | null> {
    try {
      const publicConnectorUrlResponse = await getConnectorPublicUrl();
      const connectorUrl = publicConnectorUrlResponse?.url;
      ...
      const response = await axios.get(`${normalizedUrl}/api/v1/records/${recordId}`);
      ...
      console.error('Connector fallback failed while fetching record details:', error);
      return null;
    }
  }
  ...
  static async getRecordDetails(recordId: string): Promise<RecordDetailsResponse> {
    try {
      ...
      console.warn('Primary record details fetch failed, attempting connector fallback', error);
      const fallback = await KnowledgeBaseAPI.fetchRecordDetailsFromConnector(recordId);
  ...
  // KB Linking operations - exposed by Node backend under /api/v1/knowledgeBase
  private static readonly KB_API_BASE = API_BASE;

  /**
   * Link a knowledge base to another knowledge base
   * @param kbId - The ID of the knowledge base to link to
   * @param linkedKbId - The ID of the knowledge base to be linked
   */
  static async linkKnowledgeBase(kbId: string, linkedKbId: string): Promise<void> {
    const url = `${this.KB_API_BASE}/${kbId}/links`;
    const response = await axios.post(url, { linkedKbId });
    if (!response.data?.success) {
      throw new Error(response.data?.reason || 'Failed to link knowledge base');
    }
  }

  /**
   * Unlink a knowledge base from another knowledge base
   * @param kbId - The ID of the knowledge base to unlink from
   * @param linkedKbId - The ID of the knowledge base to be unlinked
   */
  static async unlinkKnowledgeBase(kbId: string, linkedKbId: string): Promise<void> {
    const url = `${this.KB_API_BASE}/${kbId}/links/${linkedKbId}`;
    const response = await axios.delete(url);
    if (!response.data?.success) {
      throw new Error(response.data?.reason || 'Failed to unlink knowledge base');
    }
  }

  /**
   * Get all linked knowledge bases for a given knowledge base
   * @param kbId - The ID of the knowledge base
   */
  static async getLinkedKnowledgeBases(kbId: string): Promise<any[]> {
    const url = `${this.KB_API_BASE}/${kbId}/links`;
    const response = await axios.get(url);
    if (!response.data?.success) {
      throw new Error(response.data?.reason || 'Failed to fetch linked knowledge bases');
    }
    return response.data.linkedKBs || [];
  }

  /**
   * Get contributors (users who have uploaded files) for a knowledge base
   * @param kbId - The ID of the knowledge base
   */
  static async getKBContributors(kbId: string): Promise<any[]> {
    const response = await axios.get(`${this.KB_API_BASE}/${kbId}/contributors`);
    if (!response.data?.success) {
      throw new Error(response.data?.reason || 'Failed to fetch KB contributors');
    }
