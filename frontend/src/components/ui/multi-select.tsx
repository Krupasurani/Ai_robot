...
interface MultipleSelectorProps {
  ...
  /** Custom render function for option label (dropdown and badge). */
...
function removePickedOption(groupOption: GroupOption, picked: Option[]) {
  ...
  Object.entries(cloneOption).forEach(([key, value]) => {
...
>(({ className, ...props }, forwardedRef) => {
  ...
  return (
    <div
      ...
      // Removed unknown property 'cmdk-empty'
      role="presentation"
      {...props}
    />
  );
...
const MultipleSelector = React.forwardRef<MultipleSelectorRef, MultipleSelectorProps>(
  (
    ...
    const commandFilter = React.useCallback(() => {
      ...
      if (creatable) {
        return (optionValue: string, search: string) => optionValue.toLowerCase().includes(search.toLowerCase()) ? 1 : -1;
      }
    ...
    return (
      ...
      >
        <form autoComplete="off" onSubmit={(e) => e.preventDefault()}>
          <div
            className={cn(
              'min-h-10 rounded-sm border border-input text-base ring-offset-background md:text-sm',
              {
          ...
          >
            <div className="relative flex flex-wrap gap-1">
              {selected.map((option) => (
                <Badge
                  key={option.value}
                  className={cn('bg-primary text-whit', badgeClassName)}
                ...
                >
                  ...
                  >
                    <X className="h-3 w-3 text-white hover:text-foreground cursor-pointer" />
                  </button>
                </Badge>
              ))}
              ...
              <button
                ...
                className={cn(
                  'absolute ltr:right-0 rtl:left-0 h-6 w-6 p-0 cursor-pointer',
                  (hideClearAllButton ||
                    disabled ||
                    selected.length < 1 ||
