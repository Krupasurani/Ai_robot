...
// Cleanup function to restore body state after dialog closes
...
function Dialog({
  ...
  React.useEffect(() => {
    // When dialog closes (was open, now closed)
    if (prevOpenRef.current && !open) {
      // Cleanup immediately
      cleanupBodyStyles();
      // And after delays to catch Radix's internal state changes
      const timeout1 = setTimeout(cleanupBodyStyles, 50);
      const timeout2 = setTimeout(cleanupBodyStyles, 150);
      const timeout3 = setTimeout(cleanupBodyStyles, 300);
  ...
  // Wrap onOpenChange to ensure cleanup
