...
export const authInitConfig = async (email: string): Promise<AuthInitResponse> => {
  try {
    ...
    // Store the device ID if returned (in case server generated one)
    if ((response.data as any).deviceId) {
      localStorage.setItem('deviceId', (response.data as any).deviceId);
    }
...
export const getUserSessions = async (): Promise<SessionsResponse> => {
  try {
    const response = await axios.get<SessionsResponse>(
      `${CONFIG.authUrl}/api/v1/userAccount/sessions`
    );
    return response.data;
  } catch (error) {
...
export const deleteSession = async (sessionId: string): Promise<void> => {
  try {
    await axios.delete(`${CONFIG.authUrl}/api/v1/userAccount/sessions/${sessionId}`);
  } catch (error) {
    throw new Error('Error deleting session');
  }
};

export const deleteAllOtherSessions = async (): Promise<void> => {
  try {
    await axios.delete(`${CONFIG.authUrl}/api/v1/userAccount/sessions`);
  } catch (error) {
    throw new Error('Error deleting all other sessions');
  }
};
