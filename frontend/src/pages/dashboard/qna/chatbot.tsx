...
function PageContent() {
  ...
  const fetchOnboardingStatus = useCallback(async () => {
    try {
      ...
      const status = response.data?.status || response.data?.onboardingStatus || 
                     (typeof response.data === 'string' ? response.data : null);

      if (!status) {
        // If no status found, assume not configured for admins, configured for others
        console.warn('No status found in onboarding response, using default');
      }

      // Store the status in state (use 'configured' as default if not found)
      setOnboardingStatus(status || 'configured');

      // Only show the configuration dialog if:
      // 1. Status is notConfigured (explicit)
      // 2. User is an admin
      if (status === 'notConfigured' && isAdmin) {
        ...
        if (user && (!user.fullName || user.fullName.trim() === '')) {
          ...
          toast.info("Please set your full name to continue")
        }
      }
