...
import { CrawlingManagerContainer } from './modules/connector_scheduler/container/cs_container';
import createCrawlingManagerRouter from './modules/connector_scheduler/routes/cs_routes';
...
import { initializeAIModelsFromEnv } from './modules/configuration_manager/services/ai-models-initializer.service';
import { KeyValueStoreService } from './libs/services/keyValueStore.service';
import { createLicenseRouter } from './modules/license/routes/license.routes';
import { createPromptLibraryRouter } from './modules/prompt_library/routes/prompt.routes';
...
export class Application {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      const configurationManagerKeyValueService =
        this.configurationManagerContainer.get<KeyValueStoreService>(
          'KeyValueStoreService',
        );

      await initializeAIModelsFromEnv({
        keyValueStoreService: configurationManagerKeyValueService,
        configurationManagerConfig,
        appConfig,
        logger: this.logger,
      });
      ...
      // Frontend static serving / fallback
      const isProd = process.env.NODE_ENV === 'production';
      if (isProd) {
        // In production, serve the built Vite frontend from frontend/dist
        ...
        // In development, redirect unknown routes to the Vite dev server
        const frontendUrl = process.env.FRONTEND_PUBLIC_URL || 'http://localhost:3001';
  ...
  private configureRoutes(): void {
    ...
    this.app.use(
      '/api/v1/projects',
      createProjectsRouter(this.entityManagerContainer),
    );
    this.app.use(
      '/api/v1/prompts',
      createPromptLibraryRouter(this.entityManagerContainer),
    );
    this.app.use(
      '/api/v1/userGroups',
    ...
    this.app.use(
      '/api/v1/chat',
      createUnifiedChatRouter(this.esAgentContainer),
    );
    this.app.use(
      '/api/chat',
      createUnifiedChatRouter(this.esAgentContainer),
    ...
    // license & billing routes (proxy to central license-server)
    this.app.use(
      '/api/v1/license',
      createLicenseRouter(this.entityManagerContainer),
    );
  ...
  async runMigration(): Promise<void> {
    try {
      ...
      const migrationService = this.configurationManagerContainer.get<MigrationService>(MigrationService);
  ...
  private setupSwagger() {
    try {
      ...
      const swaggerConfig: SwaggerConfig = {
        title: 'Thero API',
        version: '1.0.0',
        description: 'RESTful API for Thero services',
        contact: {
          name: 'API Support',
          email: 'contact@thero.com',
        },
        basePath: '/api-docs',
      };
