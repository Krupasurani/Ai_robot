...
export class EncryptionService {
  ...
  /**
   * Checks if the given text appears to be in encrypted format (iv:ciphertext:authTag)
   */
  public isEncryptedFormat(text: string): boolean {
    const parts = text.split(':');
    if (parts.length !== 3) {
      return false;
    }
    // Check if all parts look like hex strings (valid encrypted format)
    const hexPattern = /^[0-9a-fA-F]+$/;
    return parts.every((part) => hexPattern.test(part));
  }

  /**
   * Safely decrypts text, returning the original if it's not in encrypted format.
   * Use this for backward compatibility with data that may be stored unencrypted.
   */
  public safeDecrypt(text: string | null | undefined): string {
    ...
    // If not in encrypted format, assume it's plain text and return as-is
    if (!this.isEncryptedFormat(text)) {
      logger.warn(
        'Data is not in encrypted format; returning as plain text. Consider re-encrypting stored data.',
      );
      return text;
    }
