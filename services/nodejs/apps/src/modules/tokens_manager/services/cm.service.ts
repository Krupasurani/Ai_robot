...
} from '../../configuration_manager/utils/util';
...
export class ConfigService {
  ...
  public async getSearxngBackendUrl(): Promise<string> {
    const url =
      (await this.keyValueStoreService.get<string>(configPaths.endpoint)) ||
      '{}';
    ...
    parsedUrl.webSearch = parsedUrl.webSearch || {};
    parsedUrl.webSearch.searxng = {
      ...parsedUrl.webSearch.searxng,
      endpoint:
        parsedUrl.webSearch?.searxng?.endpoint ||
    ...
    return parsedUrl.webSearch.searxng.endpoint;
  }

  public async getFirecrawlBackendUrl(): Promise<string> {
    const url =
      (await this.keyValueStoreService.get<string>(configPaths.endpoint)) ||
      '{}';
    ...
    parsedUrl.webSearch = parsedUrl.webSearch || {};
    parsedUrl.webSearch.firecrawl = {
      ...parsedUrl.webSearch.firecrawl,
      endpoint:
        parsedUrl.webSearch?.firecrawl?.endpoint ||
    ...
    return parsedUrl.webSearch.firecrawl.endpoint;
  }

  public async getPlatformSettingsConfig(): Promise<PlatformSettings> {
    try {
      return await getPlatformSettingsFromStore(this.keyValueStoreService);
    } catch (error) {
      return {
  ...
  public async getLicenseServiceConfig(): Promise<{ apiKey: string; hmacSecret: string }> {
    const fallback = {
      apiKey: process.env.LICENSE_SERVER_API_KEY || '',
      hmacSecret: process.env.LICENSE_SERVER_HMAC_SECRET || '',
    };
    return this.getEncryptedConfig<{ apiKey: string; hmacSecret: string }>(
      configPaths.license.credentials,
      fallback,
    );
  }

  /**
   * License Server backend URL
   * - Stored in the shared endpoints config under `license`
   * - Falls back to LICENSE_BACKEND / LICENSE_PORT for local development
   */
  public async getLicenseBackendUrl(): Promise<string> {
    const url =
      (await this.keyValueStoreService.get<string>(configPaths.endpoint)) ||
      '{}';
    ...
    parsedUrl.license = {
      ...parsedUrl.license,
      endpoint:
        parsedUrl.license?.endpoint ||
        process.env.LICENSE_BACKEND ||
        `http://localhost:${process.env.LICENSE_PORT ?? 8080}`,
    };
    ...
    return parsedUrl.license.endpoint;
  }
  ...
  public async getScopedJwtSecret(): Promise<string> {
    ...
    if (!parsedKeys.scopedJwtSecret) {
      parsedKeys.scopedJwtSecret =
        process.env.SCOPED_JWT_SECRET || randomKeyGenerator();
      const encryptedKeys = this.encryptionService.encrypt(
        JSON.stringify(parsedKeys),
      );
      await this.keyValueStoreService.set(
  ...
  public async getCookieSecret(): Promise<string> {
    ...
    if (!parsedKeys.cookieSecret) {
      parsedKeys.cookieSecret =
        process.env.COOKIE_SECRET || randomKeyGenerator();
      const encryptedKeys = this.encryptionService.encrypt(
        JSON.stringify(parsedKeys),
      );
      await this.keyValueStoreService.set(
