...
import type { PlatformSettings } from '../../configuration_manager/utils/util';
...
export interface AppConfig {
  ...
  authBackend: string;
  cmBackend: string;
  licenseBackend: string;
  licenseBackendApiKey?: string;
  licenseBackendHmacSecret?: string;
  kbBackend: string;
  esBackend: string;
  storageBackend: string;
  tokenBackend: string;
  aiBackend: string;
  searxngBackend: string;
  firecrawlBackend: string;
...
export const loadAppConfig = async (): Promise<AppConfig> => {
  ...
  const licenseSecrets = await configService.getLicenseServiceConfig();
  ...
  return {
    ...
    licenseBackend: await configService.getLicenseBackendUrl(),
    licenseBackendApiKey: licenseSecrets.apiKey,
    licenseBackendHmacSecret: licenseSecrets.hmacSecret,
    ...
    searxngBackend: await configService.getSearxngBackendUrl(),
    firecrawlBackend: await configService.getFirecrawlBackendUrl(),
    ...
    platformSettings: await configService.getPlatformSettingsConfig(),
  };
};
