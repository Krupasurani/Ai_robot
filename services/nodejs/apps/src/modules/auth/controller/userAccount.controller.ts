...
export class UserAccountController {
  ...
  async verifyOTP(
    ...
    if (userCredentials.isBlocked) {
      throw new BadRequestError(
        'Your account has been disabled as you have entered incorrect OTP/Password too many times. Please reach out to your admin or reachout to contact@thero.com',
      );
    }
  ...
  async initAuth(
    ...
    try {
      ...
      const session = await this.sessionService.createSession({
        ...
        currentStep: 0,
        // Device information for multiple session support
        deviceId: deviceId || uuidv4(), // Generate if not provided
      ...
      res.json({
        ...
        deviceId: session.deviceId, // Return the device ID for future requests
      });
    } catch (error) {
      next(error);
    }
  ...
  // Helper method to parse device name from user agent
  ...
  async sendForgotPasswordEmail(user: Record<string, any>) {
    try {
      ...
      await this.mailService.sendMail({
        ...
        subject: 'Thero | Reset your password!',
        templateData: {
          orgName: org?.shortName || org?.registeredName,
          name: user.fullName,
  ...
  async generateAndSendLoginOtp(
    ...
    if (userCredentialData?.isBlocked) {
      throw new ForbiddenError(
        'OTP not sent. You have entered incorrect OTP/Password too many times. Your account has been disabled. Please reach out to your admin or reachout to contact@thero.com to get it restored.',
      );
    }
  ...
  async getAccessTokenFromRefreshToken(
    ...
    try {
      ...
      if (userCredential.isBlocked) {
        throw new BadRequestError(
          'Your account has been disabled. If it is a mistake, Please reach out to contact@thero.com to get it restored.',
        );
      }

      const accessToken = await generateAuthToken(user, this.config.jwtSecret, sessionId);
  ...
  // Multiple session management methods
  ...
  async getUserSessions(
    ...
    try {
      ...
      const formattedSessions = sessions.map(session => ({
        ...
        isCurrentSession: session.token === req.user?.sessionId,
      }));
  ...
  async deleteSession(
    ...
    try {
      ...
      // Prevent user from deleting their current session
      ...
      const sessions = await this.sessionService.getUserSessions(userId);
      const sessionToDelete = sessions.find(s => s.token === sessionId);
      ...
      // Send real-time notification to the deleted session's user
      const notificationService = NotificationService.getInstance();
      if (notificationService) {
        notificationService.sendToUser(userId, 'session:invalidated', {
          sessionId: sessionId,
  ...
  async deleteAllOtherSessions(
    ...
    try {
      ...
      // If no sessionId in token (legacy token), there are no sessions to delete
      if (!currentSessionId) {
        res.status(200).json({
          message: 'No active sessions to delete. Please re-login to enable session management.',
          sessionsDeleted: 0,
        });
        return;
      }

      // Get all sessions before deletion to send notifications
      const allSessions = await this.sessionService.getUserSessions(userId);
      
      await this.sessionService.deleteOtherUserSessions(userId, currentSessionId);

      // Send real-time notification to all deleted sessions
      const notificationService = NotificationService.getInstance();
      if (notificationService) {
        for (const session of allSessions) {
          if (session.token && session.token !== currentSessionId) {
            notificationService.sendToUser(userId, 'session:invalidated', {
              sessionId: session.token,
              reason: 'All other sessions deleted by user',
              timestamp: Date.now(),
            });
          }
        }
