...
export function createUserRouter(container: Container) {
  ...
  // Dynamic middleware getters to always use the latest bound instance
  const dynamicAuth = () =>
  ...
  const dynamicScoped = (scope: TokenScopes) =>
    (req: Request, res: Response, next: NextFunction) =>
      container
        .get<AuthMiddleware>('AuthMiddleware')
        .scopedTokenValidator(scope)(req as any, res, next);
  ...
  router.get(
    ...
    async (
      ...
      try {
        const userController = container.get<UserController>('UserController');
        await userController.getUsersGraphList(req, res, next);
      } catch (error) {
        next(error);
      }
    },
  ...
  router.get(
    '/email/exists',
    metricsMiddleware(container),
    dynamicScoped(TokenScopes.USER_LOOKUP),
  ...
  router.get(
    ...
    async (
      ...
      try {
        const userController = container.get<UserController>('UserController');
        await userController.getUserDisplayPictureById(req, res, next);
      } catch (error) {
        next(error);
      }
    },
  ...
  router.post(
    '/updateAppConfig',
    dynamicScoped(TokenScopes.FETCH_CONFIG),
    async (
      ...
      try {
        ...
        // Rebind AuthMiddleware with updated secrets so subsequent requests verify with latest keys
        const updatedAuthTokenService = new AuthTokenService(
          updatedConfig.jwtSecret,
          updatedConfig.scopedJwtSecret,
        );
        const updatedAuthMiddleware = new AuthMiddleware(
          logger,
          updatedAuthTokenService,
        );
        container
          .rebind<AuthMiddleware>('AuthMiddleware')
          .toConstantValue(updatedAuthMiddleware);
