...
import { LicenseHttpService } from '../../license/services/license.service';
...
export class UserController {
  constructor(
    ...
    this.licenseService = new LicenseHttpService(this.config);
  }
  private licenseService: LicenseHttpService;
  ...
  async getUsersGraphList(
    ...
    try {
      ...
      const userRecords = (await Users.find(
        ...
        'fullName firstName lastName email hasLoggedIn createdAt updatedAt userId',
      )
      ...
      const users = userRecords.map((user) => {
        ...
        return {
          id,
          userId: String(user.userId ?? id),
          name: derivedName,
          email: user.email,
          isActive: Boolean(user.hasLoggedIn),
          createdAtTimestamp: user.createdAt?.getTime(),
          updatedAtTimestamp: user.updatedAt?.getTime(),
        };
      });
  ...
  async createUser(
    ...
    try {
      ...
      try {
        await this.ensureSeatAllocation(
          orgId.toString(),
  ...
  async provisionSamlUser(
    ...
    try {
      await this.ensureSeatAllocation(orgId.toString(), (newUser._id as any).toString(), email);
    } catch (error) {
      await Users.deleteOne({ _id: newUser._id });
      throw error;
    }
  ...
  async updateUser(
    ...
    try {
      ...
      await this.releaseSeatAllocation(user.orgId.toString(), (user._id as any).toString());
  ...
  async updateUserDisplayPicture(
    ...
    try {
      ...
      // Publish userUpdated event to sync avatar to ArangoDB
      try {
        const user = await Users.findById(userId).select('email').lean();
        if (user && orgId && userId) {
          const event: Event = {
            eventType: EventType.UpdateUserEvent,
            timestamp: Date.now(),
            payload: {
              ...
              email: user.email || '',
              photoBase64: compressedPic,
              photoMimeType: compressedPicMimeType,
            } as UserUpdatedEvent,
          };
          await this.eventService.publishEvent(event);
          this.logger.info(`Published avatar update event for user ${userId}`);
        }
      } catch (eventError) {
        this.logger.error('Failed to publish avatar update event', eventError);
  ...
  async getUserDisplayPictureById(
    req: AuthenticatedUserRequest,
    res: Response,
    next: NextFunction,
  ): Promise<void> {
        ...
        for (const user of newUsers) {
          if (!user?._id) {
            throw new InternalServerError(
              'User ID missing while ensuring license seat for new user.',
            );
          }
          try {
            await this.ensureSeatAllocation(
  ...
  private async ensureSeatAllocation(orgId: string, userId: string, email: string) {
    try {
      await this.licenseService.ensureSeat(orgId, userId, email);
    } catch (error) {
      this.logger.error('Failed to ensure license seat', {
        orgId,
        userId,
  ...
  private async releaseSeatAllocation(orgId: string, userId: string) {
    try {
      await this.licenseService.releaseSeat(orgId, userId);
    } catch (error) {
      this.logger.warn('Failed to release license seat', {
        orgId,
        userId,
        error: (error as Error).message,
      });
