  ...
  static async dispose(): Promise<void> {
    if (this.instance) {
      try {
        ...
        if (this.instance.isBound('RedisService')) {
          const redisService = this.instance.get<RedisService>('RedisService');
          if (redisService.isConnected()) {
            await redisService.disconnect();
            this.logger.info('RedisService disconnected successfully');
          }
        }
