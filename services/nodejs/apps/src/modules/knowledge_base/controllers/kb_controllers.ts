...
export const createKnowledgeBase =
  ...
  async (
    ...
    try {
      ...
      const { kbName, icon, description } = req.body;
...
/**
 * Link another knowledge base to this Knowledge Base
 * Proxies to connector backend: POST /api/v1/kb/:kbId/links
 */
export const linkKnowledgeBaseToKnowledgeBase =
  (appConfig: AppConfig) =>
  async (
    req: AuthenticatedUserRequest,
    res: Response,
    ...
    try {
      ...
      if (!linkedKbId) {
        throw new BadRequestError('linkedKbId is required');
      }

      const response = await executeConnectorCommand(
        `${appConfig.connectorBackend}/api/v1/kb/${kbId}/links`,
        HttpMethod.POST,
        req.headers as Record<string, string>,
        { linkedKbId },
      );

      // Allow both 200 and 201 from connector backend
      if (response && ![200, 201].includes(response.statusCode)) {
        const handled = handleBackendError(response, 'link knowledge base');
        return next(handled);
      }
      if (!response?.data) {
        return next(new NotFoundError('Linking knowledge base failed: empty response'));
      }

      res.status(200).json(response.data);
    } catch (error: any) {
      logger.error('Error linking knowledge bases', {
        kbId: req.params.kbId,
        userId: req.user?.userId,
        error: error.message,
      ...
      let handleError: any;
      try {
        handleError = handleBackendError(error, 'link knowledge base');
      } catch (e: any) {
        handleError = e;
      }
      next(handleError);
...
/**
 * Unlink a knowledge base from this Knowledge Base
 * Proxies to connector backend: DELETE /api/v1/kb/:kbId/links/:linkedKbId
 */
export const unlinkKnowledgeBaseFromKnowledgeBase =
  (appConfig: AppConfig) =>
  async (
    req: AuthenticatedUserRequest,
    res: Response,
    ...
    try {
      const { userId } = req.user || {};
      const { kbId, linkedKbId } = req.params;
      ...
      if (!linkedKbId) {
        throw new BadRequestError('Linked Knowledge Base ID is required');
      }

      const response = await executeConnectorCommand(
        `${appConfig.connectorBackend}/api/v1/kb/${kbId}/links/${linkedKbId}`,
        HttpMethod.DELETE,
        req.headers as Record<string, string>,
      );

      if (response && response.statusCode !== 200) {
        throw handleBackendError(response, 'unlink knowledge base');
      }
      if (!response?.data) {
        throw new NotFoundError('Unlinking knowledge base failed: empty response');
      }

      res.status(200).json(response.data);
    } catch (error: any) {
      logger.error('Error unlinking knowledge bases', {
        kbId: req.params.kbId,
        linkedKbId: req.params.linkedKbId,
        userId: req.user?.userId,
        error: error.message,
        requestId: req.context?.requestId,
      });
      const handleError = handleBackendError(error, 'unlink knowledge base');
      next(handleError);
    }
  };

/**
 * Get linked knowledge bases for a Knowledge Base
 * Proxies to connector backend: GET /api/v1/kb/:kbId/links
 */
export const getLinkedKnowledgeBases =
  (appConfig: AppConfig) =>
  async (
    req: AuthenticatedUserRequest,
    res: Response,
    ...
    try {
      ...
      if (response && response.statusCode !== 200) {
        throw handleBackendError(response, 'get linked knowledge bases');
      }
      if (!response?.data) {
        throw new NotFoundError('Fetching linked knowledge bases failed: empty response');
      }

      res.status(200).json(response.data);
    } catch (error: any) {
      logger.error('Error fetching linked knowledge bases', {
        kbId: req.params.kbId,
        userId: req.user?.userId,
        error: error.message,
        requestId: req.context?.requestId,
      });
      const handleError = handleBackendError(error, 'get linked knowledge bases');
      next(handleError);
    }
  };
