...
import { maybeGenerateConversationTitle } from './conversation-title.utils';
...
/**
 * Build an image generation response message for storing in conversation history.
 */
export const buildImageResponseMessage = (
  imageBase64: string,
  mimeType: string,
  model?: string,
  textResponse?: string,
...
export const buildAIResponseMessage = (
  ...
  console.log('buildAIResponseMessage received:', JSON.stringify(aiResponse, null, 2));
  
  // Handle clarification requests specially (low confidence - needs more info from user)
  if (aiResponse?.data?.isClarificationRequest) {
    console.log('Building clarification message');
    return {
      messageType: 'clarification',
      createdAt: new Date(),
      updatedAt: new Date(),
      content: aiResponse.data.answer || aiResponse.data.question || 'Please provide more details.',
      contentFormat: 'MARKDOWN',
      citations: [],
      confidence: '',
      followUpQuestions: [],
      metadata: {
        reason: aiResponse.data?.clarificationReason || aiResponse.data?.reason,
        originalQuery: aiResponse.data?.originalQuery,
        isClarificationRequest: true,
      },
    };
  }
  
  // Handle research plan confirmation requests (medium confidence - needs user to confirm plan)
  if (aiResponse?.data?.isPlanConfirmation) {
    console.log('Building research plan confirmation message');
    return {
      messageType: 'research_plan',
      createdAt: new Date(),
      updatedAt: new Date(),
      content: '', // No content yet - plan is shown in the frontend card
      contentFormat: 'MARKDOWN',
      citations: [],
      confidence: '',
      followUpQuestions: [],
      metadata: {
        reason: aiResponse.data?.reason || 'plan_awaiting_confirmation',
        originalQuery: aiResponse.data?.originalQuery,
        isPlanConfirmation: true,
        researchPlan: aiResponse.data?.researchPlan || [],
        assumptions: aiResponse.data?.assumptions || [],
        identifiedGaps: aiResponse.data?.identifiedGaps || [],
        confidenceScore: aiResponse.data?.confidenceScore || 0,
        confidenceLevel: aiResponse.data?.confidenceLevel || 'medium',
  ...
  if (!aiResponse?.data?.answer) {
    console.log('Validation failed - aiResponse structure:', {
      hasAiResponse: !!aiResponse,
      hasData: !!aiResponse?.data,
      hasAnswer: !!aiResponse?.data?.answer,
      dataKeys: aiResponse?.data ? Object.keys(aiResponse.data) : 'no data'
    });
...
export const buildSharedWithMeFilter = (req: AuthenticatedUserRequest) => {
  ...
  const filter = {
    ...
    //    (public-org sharing via isShared alone is NOT sufficient for access)
...
export const buildAgentSharedWithMeFilter = (
  ...
  return {
    ...
    isDeleted: false,
    isArchived: false,
    initiator: { $ne: new mongoose.Types.ObjectId(`${userId}`) },
    $or: [
      {
...
export const buildFilter = (
  ...
  if (req.query.projectId) {
    try {
      ...
      throw new BadRequestError('Invalid projectId format');
    }
  }

  // Exclude project conversations when requested (ensure AND with base filter)
  if (req.query.excludeProject === 'true') {
    filter.$and = [
      ...(filter.$and || []),
      {
        $or: [
...
export const buildAgentConversationFilter = (
  ...
  const filter = buildFilter(req, orgId, userId, id as string | undefined);
  return {
    ...filter,
    agentKey,
  };
...
export const saveCompleteConversation = async ({
  ...
  session,
  requestId,
}: SaveConversationOptions): Promise<{
  conversation: IConversationWithId;
  ...
  try {
    ...
    // Set appropriate status - clarification requests should not be marked as COMPLETE
    if (completeData.isClarificationRequest) {
    ...
    const plainConversation = updatedConversation.toObject() as IConversationWithId;
    const citationMap = new Map(citations.map((c: ICitation) => [c._id?.toString(), c]));

    const sanitizedConversation: IConversationWithId = {
      ...plainConversation,
      messages: plainConversation.messages
...
export const saveCompleteAgentConversation = async (
  ...
  return saveCompleteConversation(options);
};
...
export const markAgentConversationFailed = async (
  ...
  return markConversationFailed(conversation, failReason, session);
};
...
export const deleteAgentConversation = async (
  ...
  return {
    id: conversationId,
    status: 'deleted',
    deletedAt: updatedConversation.updatedAt,
    deletedBy: userId,
    citationsDeleted: citationIds.length,
  };
};
