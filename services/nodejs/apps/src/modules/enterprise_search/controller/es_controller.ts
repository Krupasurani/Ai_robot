...
} from '../utils/conversation-title.utils';
...
const resolveConversationLanguage = (
  ...
  const headerValue = req.context?.headers.language
    || (Array.isArray(acceptLanguage)
      ? acceptLanguage[0]
      : (acceptLanguage as string | undefined));
  return normalizeLanguage(headerValue);
};
...
const buildWebSearchPayload = (
  ...
  const config = (body?.webSearch as Record<string, any>) || {};
  const enabled =
    typeof body?.useWebSearch === 'boolean'
      ? body.useWebSearch
      : (config.enabled as boolean | undefined) ?? false;

  return {
    ...config,
    enabled,
    mode: config.mode || (enabled ? 'always' : 'auto'),
    maxResults: config.maxResults ?? 5,
    maxPages: config.maxPages ?? 2,
    includeSourceContent: config.includeSourceContent ?? true,
    firecrawlStrategy: config.firecrawlStrategy || 'fast',
    endpoints: {
      searxng: config.endpoints?.searxng || appConfig.searxngBackend,
      firecrawl: config.endpoints?.firecrawl || appConfig.firecrawlBackend,
    },
  };
};

type UnifiedChatMode = 'agent' | 'knowledge' | 'chat' | 'deepResearch' | 'image';

const LEGACY_CHAT_MODE_MAP: Record<string, UnifiedChatMode> = {
  science: 'knowledge',
  search: 'deepResearch',
};

const normalizeChatMode = (mode?: string): UnifiedChatMode => {
  if (!mode || typeof mode !== 'string') {
    return 'chat';
  }
  const lowered = mode.toLowerCase();
  if (LEGACY_CHAT_MODE_MAP[lowered]) {
    return LEGACY_CHAT_MODE_MAP[lowered];
  }
  if (lowered === 'deepresearch') {
    return 'deepResearch';
  }
  if (['agent', 'knowledge', 'chat', 'deepresearch', 'image'].includes(lowered)) {
    return lowered === 'deepresearch' ? 'deepResearch' : (lowered as UnifiedChatMode);
  }
  return 'chat';
};
...
const mapUnifiedChatPayload = (
  payload: Record<string, any>,
  forcedMode?: UnifiedChatMode,
) => {
  const normalizedMode = forcedMode ?? normalizeChatMode(payload?.mode);
  const apps = sanitizeStringArray(payload?.filters?.apps);
  const kb = sanitizeStringArray(payload?.filters?.kb);
  const filters =
    apps || kb
      ? {
          ...(apps ? { apps } : {}),
            ...
            return {
              ...
              content:
                typeof attachment.content === 'string' && attachment.content.length > 0
                  ...
                  : undefined,
              processingTimeMs:
                typeof attachment.processingTimeMs === 'number'
                  ? attachment.processingTimeMs
                  : undefined,
            };
          })
          .filter(Boolean)
  ...
  return {
    ...
    conversationId:
      typeof payload?.conversationId === 'string' && payload.conversationId.trim().length > 0
        ...
        : undefined,
    chatMode: typeof payload?.chatMode === 'string' ? payload.chatMode : undefined,
    // Unified quick-mode behavior:
    // - Frontend toggles a "quick" chatMode to indicate lightweight behavior
    // - Backend should see this as `quickMode: true`
    quickMode:
      typeof payload?.chatMode === 'string' &&
      payload.chatMode.trim().toLowerCase() === 'quick',
    modelKey:
      typeof payload?.model?.key === 'string' && payload.model.key.trim().length > 0
        ? payload.model.key
        : undefined,
    modelName:
      typeof payload?.model?.name === 'string' && payload.model.name.trim().length > 0
        ? payload.model.name
        : undefined,
    useReasoning: Boolean(payload?.reasoningEnabled),
    useWebSearch:
      payload?.webSearchEnabled !== undefined
        ? Boolean(payload.webSearchEnabled)
        : normalizedMode === 'deepResearch',
    filters,
    recordIds: sanitizeStringArray(payload?.context?.recordIds) || [],
    projectId:
      typeof payload?.context?.projectId === 'string' && payload.context.projectId.trim().length > 0
    ...
    isClarificationResponse: Boolean(payload?.isClarificationResponse),
    // Deep Research plan confirmation fields
    isPlanConfirmation: Boolean(payload?.isPlanConfirmation),
    // Original query for clarification/plan confirmation flows
    originalQuery:
      typeof payload?.originalQuery === 'string' && payload.originalQuery.trim().length > 0
        ? payload.originalQuery
        : undefined,
    // Confirmed research plan and assumptions
    confirmedPlan: Array.isArray(payload?.confirmedPlan) ? payload.confirmedPlan : undefined,
    confirmedAssumptions: Array.isArray(payload?.confirmedAssumptions) ? payload.confirmedAssumptions : undefined,
    // Research scope for deep research
    researchScope:
      typeof payload?.researchScope === 'string' && payload.researchScope.trim().length > 0
...
export const streamChat =
  ...
  async (req: AuthenticatedUserRequest, res: Response) => {
    ...
    const preferredLanguage = resolveConversationLanguage(req);
    const placeholderTitle = getPlaceholderTitle(preferredLanguage);
    ...
    try {
      ...
      const userConversationData: Partial<IConversation> = {
        ...
        titleLanguage: preferredLanguage,
      ...
      // Prepare AI payload
      const isQuickChatMode =
        typeof req.body.chatMode === 'string' &&
        req.body.chatMode.trim().toLowerCase() === 'quick';
      ...
      const aiPayload = {
        ...
        // Do NOT forward modelKey/modelName to the Python chatbot – it will use its own defaults.
        // Quick-mode behavior is controlled via a dedicated boolean flag.
        quickMode: Boolean(req.body.quickMode) || isQuickChatMode,
        // Web search is toggled via a simple flag; detailed config stays in the AI service.
        useWebSearch: req.body.useWebSearch ?? false,
        mode: req.body.mode || 'chat',
        // Expose reasoning toggle explicitly so the Python service can react if needed.
        reasoningEnabled: Boolean(req.body.useReasoning),
        attachments: req.body.attachments || [],
        projectId: req.body.projectId || null,
        // Propagate the resolved conversation language so the AI backend can
        // steer web search towards the user's locale when helpful.
        language: preferredLanguage,
        // Deep Research clarification/plan confirmation fields
        isClarificationResponse: req.body.isClarificationResponse || false,
        isPlanConfirmation: req.body.isPlanConfirmation || false,
        originalQuery: req.body.originalQuery || null,
        confirmedPlan: req.body.confirmedPlan || null,
        confirmedAssumptions: req.body.confirmedAssumptions || null,
        researchScope: req.body.researchScope || null,
      };
      ...
      stream.on('end', async () => {
        ...
        try {
          ...
          if (completeData && savedConversation) {
            const { conversation, generatedTitle } =
              await saveCompleteConversation({
...
export const unifiedChatStream =
  (appConfig: AppConfig, forcedMode?: UnifiedChatMode) =>
  async (req: AuthenticatedUserRequest, res: Response, next: NextFunction) => {
    try {
      const normalizedPayload = mapUnifiedChatPayload(
        { ...(req.body || {}), ...(forcedMode ? { mode: forcedMode } : {}) },
        forcedMode,
      );
      ...
      req.body.query = normalizedPayload.query;
      req.body.filters = normalizedPayload.filters || {};
      req.body.recordIds = normalizedPayload.recordIds;
      req.body.modelKey = normalizedPayload.modelKey;
      req.body.modelName = normalizedPayload.modelName;
      req.body.chatMode = normalizedPayload.chatMode;
      req.body.useReasoning = normalizedPayload.useReasoning;
      req.body.useWebSearch = normalizedPayload.useWebSearch;
      req.body.webSearch = {
        ...(req.body.webSearch || {}),
        enabled: normalizedPayload.useWebSearch,
      };
      req.body.projectId = normalizedPayload.projectId;
      req.body.attachments = normalizedPayload.attachments;
      req.body.systemPrompt = normalizedPayload.systemPrompt;
      req.body.mode = normalizedPayload.mode;
      // Deep Research clarification/plan confirmation fields
      req.body.isClarificationResponse = normalizedPayload.isClarificationResponse;
      req.body.isPlanConfirmation = normalizedPayload.isPlanConfirmation;
      req.body.originalQuery = normalizedPayload.originalQuery;
      req.body.confirmedPlan = normalizedPayload.confirmedPlan;
      req.body.confirmedAssumptions = normalizedPayload.confirmedAssumptions;
      req.body.researchScope = normalizedPayload.researchScope;

      if (normalizedPayload.mode === 'agent') {
        if (!normalizedPayload.agentId) {
          throw new BadRequestError('agentId is required when mode is agent');
        }
        req.params.agentKey = normalizedPayload.agentId;
        if (normalizedPayload.conversationId) {
          req.params.conversationId = normalizedPayload.conversationId;
          return addMessageStreamToAgentConversation(appConfig)(req, res);
        }
        return streamAgentConversation(appConfig)(req, res);
      }

      if (normalizedPayload.mode === 'image') {
        return handleImageGeneration(appConfig)(req, res, next);
      }

      if (normalizedPayload.conversationId) {
        req.params.conversationId = normalizedPayload.conversationId;
        return addMessageStream(appConfig)(req, res);
      }

      return streamChat(appConfig)(req, res);
    } catch (error) {
      return next(error);
    }
  };

// Type for image generation response from Python service
...
/**
 * Handle image generation requests via the Python image service.
 * Supports both new image generation and image editing.
 * Creates/updates conversation history for persistence.
 */
export const handleImageGeneration =
  ...
  async (req: AuthenticatedUserRequest, res: Response, _next: NextFunction) => {
    ...
    try {
      // Extract image generation parameters from the request
      // Note: Frontend sends 'message', but we also support 'query' for direct API calls
      const {
        message,
        query,
        previousImageBase64,
        previousImageMimeType = 'image/png',
        conversationId,
      } = req.body;

      const imagePrompt = message || query;
      const hasEditImage = !!previousImageBase64;
      
      logger.info('Image generation request received', { 
        requestId, 
        userId,
        hasEditImage,
        previousImageLength: previousImageBase64?.length || 0,
      ...
      // Prepare the request to the Python image service
      const imagePayload = {
        prompt: imagePrompt.trim(),
        previous_image_base64: previousImageBase64 || null,
        previous_image_mime_type: previousImageMimeType,
      ...
      // Create or get conversation for history persistence
      ...
      try {
        if (conversationId) {
          ...
          if (conversation) {
            ...
            const userMessage = buildUserQueryMessage(imagePrompt.trim()) as IMessageDocument;
        ...
        if (!conversation) {
          ...
          const conversationData: Partial<IConversation> = {
            ...
            titleLanguage: preferredLanguage,
            titleManuallySet: false,
            messages: [userMessage] as IMessageDocument[],
            lastActivityAt: Date.now(),
            status: CONVERSATION_STATUS.INPROGRESS,
          };
          ...
          logger.debug('Created new conversation for image generation', {
            requestId,
            conversationId: conversation._id,
          });
        }
      } catch (convError: any) {
        logger.warn('Failed to create/update conversation for image generation', {
          requestId,
      ...
      try {
        ...
        const aiCommandOptions: AICommandOptions = {
          uri: `${appConfig.aiBackend}/api/v1/image/generate`,
          method: HttpMethod.POST,
          headers: {
            ...(req.headers as Record<string, string>),
            'Content-Type': 'application/json',
        ...
        const response = await new AIServiceCommand(aiCommandOptions).execute();

        if (response.statusCode >= 400) {
          const errorData = response.data as ImageGenerationResult | undefined;
          const errorMsg = errorData?.error || 'Image generation failed';
          
          // Mark conversation as failed
          if (conversation) {
            try {
              conversation.status = CONVERSATION_STATUS.FAILED;
              conversation.failReason = errorMsg;
              await conversation.save();
            } catch (saveError) {
              logger.warn('Failed to update conversation status', { requestId });
            }
          }
        ...
        const imageResult = response.data as ImageGenerationResult | undefined;

        if (imageResult?.success && imageResult?.image_base64) {
          // Save image response to conversation
          if (conversation) {
            try {
              const imageMessage = buildImageResponseMessage(
                imageResult.image_base64,
                imageResult.mime_type || 'image/png',
                imageResult.model,
                imageResult.text_response,
              ) as IMessageDocument;
              ...
              logger.debug('Saved image to conversation', {
                requestId,
                conversationId: conversation._id,
                messageCount: conversation.messages.length,
              });
            } catch (saveError: any) {
              logger.warn('Failed to save image to conversation', {
                requestId,
          ...
          res.write(`event: image\ndata: ${JSON.stringify({
            ...
            text_response: imageResult.text_response,
            timestamp: new Date().toISOString(),
          })}\n\n`);

          // Send completion event with conversation info
          ...
          // Send conversation data for frontend state sync
          if (conversation) {
          ...
          if (conversation) {
            try {
              conversation.status = CONVERSATION_STATUS.FAILED;
              conversation.failReason = imageResult?.error || 'Image generation failed';
              await conversation.save();
            } catch (saveError) {
              logger.warn('Failed to update conversation status', { requestId });
            }
        ...
        logger.error('Image generation AI service error', {
          requestId,
        ...
        if (conversation) {
          try {
            conversation.status = CONVERSATION_STATUS.FAILED;
            conversation.failReason = aiError.message;
            await conversation.save();
          } catch (saveError) {
            logger.warn('Failed to update conversation status', { requestId });
          }
...
export const performWebSearch =
  ...
  async (req: AuthenticatedUserRequest, res: Response, next: NextFunction) => {
    const aiPayload = {
      ...
      useWebSearch: req.body.useWebSearch ?? true,
      webSearch: buildWebSearchPayload(
        { ...req.body, useWebSearch: req.body.useWebSearch ?? true },
        appConfig,
      ),
    };

    const aiCommandOptions: AICommandOptions = {
      uri: `${appConfig.aiBackend}/api/v1/web-search`,
      method: HttpMethod.POST,
      headers: {
        ...(req.headers as Record<string, string>),
        'Content-Type': 'application/json',
    ...
    try {
      ...
      next(handleBackendError(error, 'Web Search'));
    }
  };
    ...
    async function createConversationUtil(
      ...
      try {
        ...
        const { conversation } = await saveCompleteConversation({
          conversation: savedConversation,
          completeData: aiResponseData.data,
          orgId: orgIdStr,
          appConfig,
          headers: req.headers as Record<string, string>,
          session,
  ...
  async (req: AuthenticatedUserRequest, res: Response) => {
    ...
    try {
      ...
      const aiPayload = {
        ...
        chatMode: req.body.chatMode || 'standard',
        useWebSearch: req.body.useWebSearch ?? false,
        webSearch: buildWebSearchPayload(req.body, appConfig),
        mode: req.body.mode || 'chat',
        reasoningEnabled: req.body.useReasoning ?? false,
        attachments: req.body.attachments || [],
        projectId: req.body.projectId || null,
        // Deep Research clarification/plan confirmation fields
        isClarificationResponse: req.body.isClarificationResponse || false,
        ...
        try {
          ...
          if (completeData && existingConversation) {
            try {
              ...
              const generatedTitle = await maybeGenerateConversationTitle({
...
export const updateTitle = async (
  ...
  try {
    ...
    const finalTitle = req.body.title.trim();
    const manualLanguage = resolveConversationLanguage(req);
    conversation.title = finalTitle;
    conversation.titleLanguage = manualLanguage;
    ...
    try {
      ...
      stream.on('end', async () => {
        ...
        try {
          ...
          if (completeData && savedConversation) {
            const { conversation, generatedTitle } =
              await saveCompleteAgentConversation({
                conversation: savedConversation,
                completeData,
                orgId: orgIdStr,
                appConfig,
    ...
    async function createConversationUtil(
      ...
      try {
        ...
        const { conversation } = await saveCompleteAgentConversation({
          conversation: savedConversation as IAgentConversationDocument,
          completeData: aiResponseData.data,
          orgId: orgIdStr,
          appConfig,
          headers: req.headers as Record<string, string>,
