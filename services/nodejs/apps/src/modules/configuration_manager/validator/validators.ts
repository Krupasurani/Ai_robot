...
const systemEmailTemplateSchema = z.object({
  ...
  subject: z
    .string()
    .max(200, { message: 'Subject must be 200 characters or fewer' })
    .optional(),
  markdown: z
    .string()
    .max(8000, { message: 'Template content limit exceeded (8k characters)' })
    .optional(),
});
...
export const platformSettingsSchema = z.object({
  body: z.object({
    ...
    systemEmailTemplate: systemEmailTemplateSchema.optional(),
  }),
});
...
export const aiModelsConfigSchema = z.object({
  body: z
    .object({
      ...
      deepresearch: z.array(modelConfigurationSchema).optional(),
      imageGeneration: z.array(modelConfigurationSchema).optional(),
    })
    .strict({
      message: 'ai models can be ocr, embedding, llm, slm, reasoning, multimodal, deepresearch, imageGeneration',
    })
    .refine(
      (data) => {
        // Ensure at least one field is present and non-empty
