...
import {
  ...
  prepareSystemEmailTemplateForSave,
} from '../utils/util';
...
const shouldSkipAiModelHealthChecks = (): boolean =>
  String(process.env.AI_MODELS_HEALTHCHECK_SKIP || '').toLowerCase() === 'true';

const logAiModelHealthCheckSkipped = (context: string): void => {
  logger.warn(
    `Skipping AI models health check for ${context} because AI_MODELS_HEALTHCHECK_SKIP is enabled.`,
  );
};
    ...
    try {
      ...
      switch (storageType.toLowerCase()) {
        ...
        case storageTypes.LOCAL.toLowerCase(): {
          const localConfig = {
            mountName: config.mountName || 'Thero',
...
export const setPlatformSettings =
  ...
  async (req: AuthenticatedUserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const { fileUploadMaxSizeBytes, featureFlags, systemEmailTemplate } = req.body;
      const actorName =
        typeof req.user.fullName === 'string' && req.user.fullName.trim()
          ? req.user.fullName.trim()
          : [req.user.firstName, req.user.lastName].filter(Boolean).join(' ').trim();
      const actor = {
        id: typeof req.user.userId === 'string' ? req.user.userId : undefined,
        email: typeof req.user.email === 'string' ? req.user.email : undefined,
        name: actorName || undefined,
      };
      const normalizedTemplate = prepareSystemEmailTemplateForSave(
        systemEmailTemplate,
        actor,
      );
      const configManagerConfig = loadConfigurationManagerConfig();
      const encryptedPlatformSettings = EncryptionService.getInstance(
        ...
        JSON.stringify({
          ...
          featureFlags,
          systemEmailTemplate: normalizedTemplate,
      ...
      try {
        ...
        const response = await axios(config);
        if (response.status != 200) {
          throw new BadRequestError('Error notifying mail service');
        }
      } catch (notifyError) {
        logger.warn('Failed to refresh mail configuration after saving platform settings', {
...
export const createAIModelsConfig =
  ...
  async (req: AuthenticatedUserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const skipHealthChecks = shouldSkipAiModelHealthChecks();

      // Handle LLM health check
      if (Array.isArray(aiConfig.llm) && aiConfig.llm.length > 0) {
        if (skipHealthChecks) {
          logAiModelHealthCheckSkipped('LLM configuration save');
        } else {
      ...
      if (Array.isArray(aiConfig.embedding) && aiConfig.embedding.length > 0) {
        if (skipHealthChecks) {
          logAiModelHealthCheckSkipped('embedding configuration save');
        } else {
          const aiCommandOptions: AICommandOptions = {
  ...
  async (req: AuthenticatedUserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!skipHealthChecks) {
        ...
        logAiModelHealthCheckSkipped(
          `${modelType.toUpperCase()} provider creation`,
        );
      }
  ...
  async (req: AuthenticatedUserRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!skipHealthChecks) {
        ...
        logAiModelHealthCheckSkipped(
          `${(modelType ?? 'unknown').toUpperCase()} provider update`,
        );
      }
