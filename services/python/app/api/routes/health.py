...
from libs.data.vector import ORG_ID_FIELD, VIRTUAL_RECORD_ID_FIELD
...
async def llm_health_check(request: Request, llm_configs: list[dict] = Body(...)) -> JSONResponse:
    ...
    try:
        ...
        # Use the first config (or find default)
        llm_config = next((cfg for cfg in llm_configs if cfg.get("isDefault")), llm_configs[0])
        ...
        if not model_names:
            return JSONResponse(
                ...
                content={
                    "status": "not healthy",
                    "error": "No model name found in configuration",
                    "timestamp": get_epoch_timestamp_in_ms(),
                },
            )
        ...
        logger.info(f"LLM health check: provider={provider}, model={model_name}")

        # Create LLM directly from provided config
...
async def initialize_embedding_model(request: Request, embedding_configs: list[dict]) -> Tuple[Any, Any, Any]:
    ...
    try:
        if not embedding_configs:
            raise HTTPException(
                status_code=400,
                detail="No embedding configuration provided. Please configure an OpenAI or OpenAI-compatible embedding provider."
            )
        ...
        if not dense_embeddings:
            raise HTTPException(status_code=500, detail="No embedding model could be created from configuration")
    except Exception as e:
