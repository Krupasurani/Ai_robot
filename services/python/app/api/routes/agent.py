...
def _agent_event(step: str, message: Optional[str] = None, **extra: Any) -> Dict[str, Any]:
...
async def stream_response(
    ...
    # Emit early meta event; agent graph may later emit citations-specific events
    try:
        yield f"event: meta\ndata: {json.dumps(_agent_event('meta', 'Agent session initialized', knowledgeUsed=False, retrievalStrategy='auto'))}\n\n"
    except Exception:
        # Do not fail stream on meta emission issues
        pass
