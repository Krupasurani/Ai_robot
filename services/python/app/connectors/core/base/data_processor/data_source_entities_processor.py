...
from libs.messaging import IMessagingProducer, MessagingFactory
from libs.messaging.kafka import KafkaProducerConfig
...
class DataSourceEntitiesProcessor:
    ...
    async def on_updated_record_permissions(self, record: Record, permissions: List[Permission]) -> None:
        ...
        try:
            ...
            # ACL Push-Down: Send Kafka event to sync permissions to vector store
            # This triggers the PermissionSyncService to update access_control_list in Qdrant
            await self.messaging_producer.send_message(
                "record-events",
                {
                    "eventType": EventTypes.PERMISSION_UPDATE.value,
            ...
            self.logger.info(f"📤 Sent permission update event for record: {record.id}")
