    ...
    async def batch_upsert_records(self, records: List[Record]) -> None:
        ...
        try:
            for record in records:
                ...
                record_type_str = record.record_type.value if hasattr(record.record_type, 'value') else str(record.record_type)
                collection = RECORD_TYPE_COLLECTION_MAPPING.get(record_type_str)

                # Upsert base record - this is always done for all types
                ...
                if collection:
                    ...
                    self.logger.debug(f"Record type {record_type_str} only stored in base records collection")
