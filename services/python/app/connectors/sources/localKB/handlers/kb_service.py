...
class KnowledgeBaseService :
    ...
    async def create_knowledge_base(
        ...
        try:
            ...
            # Step 1.5: Check for name conflict
            name_exists = await self.arango_service.check_kb_name_exists(
                user_id=user_key,
                org_id=org_id,
                name=name
            )
            ...
            if name_exists:
                self.logger.warning(f"⚠️ Knowledge base with name '{name}' already exists for user {user_name}")
                return {
                    "success": False,
                    "code": 409,
                    "reason": f"A knowledge base with the name '{name}' already exists."
                }
    ...
    async def delete_records_in_kb(
        ...
        try:
            ...
            if result and result.get("success"):
                success_count = result.get("successfully_deleted", 0)
                failed_count = result.get("failed_count", 0)
                total = result.get("total_requested", 0)

                message = f"Successfully deleted {success_count} of {total} records"
                if failed_count > 0:
                ...
                return {
                    "success": False,
                    "message": result.get("reason", "Failed to delete records"),
                    "deleteType": "bulk"
                }
    ...
    async def delete_records_in_folder(
        ...
        # Step 3 Call bulk deletion method (folder_id=None for KB root)
            ...
            if result and result.get("success"):
                ...
                message = f"Successfully deleted {success_count} of {total} records from folder"
                if failed_count > 0:
                    message += f" ({failed_count} failed)"
                ...
                return {
                    "success": False,
                    "message": result.get("reason", "Failed to delete records in folder"),
                    "deleteType": "bulk"
                }
    ...
    async def link_knowledge_base(
        ...
        """Link another knowledge base to this KB"""
        try:
            self.logger.info(f"🔗 Linking KB {linked_kb_id} to KB {kb_id}")

            # Validate user
            user = await self.arango_service.get_user_by_user_id(user_id=user_id)
            if not user:
                return self._error_response(404, f"User not found for user_id: {user_id}")

            user_key = user.get('_key')

            # Check user permissions on source KB (must have EDITOR or higher)
            user_role = await self.arango_service.get_user_kb_permission(kb_id, user_key)
            if user_role not in ["OWNER", "WRITER", "ORGANIZER"]:
                return self._error_response(403, "User lacks permission to link knowledge bases")

            # Prevent self-linking
            if kb_id == linked_kb_id:
                return self._error_response(400, "Cannot link a knowledge base to itself")

            # Verify linked KB exists and user has at least read access
            linked_role = await self.arango_service.get_user_kb_permission(linked_kb_id, user_key)
            if not linked_role:
                return self._error_response(403, "User has no access to the target knowledge base")

            # Check if link already exists
            existing_link = await self.arango_service.get_kb_link(kb_id, linked_kb_id)
            if existing_link:
                return self._error_response(409, "Knowledge bases are already linked")

            # Create the link
            timestamp = get_epoch_timestamp_in_ms()
            result = await self.arango_service.create_kb_link(
                source_kb_id=kb_id,
                target_kb_id=linked_kb_id,
                user_key=user_key,
                timestamp=timestamp
            )

            if result and result.get("success"):
                self.logger.info(f"✅ Successfully linked KB {linked_kb_id} to KB {kb_id}")
                return {
                    "success": True,
                    "code": 201,
                    "linkedKbId": linked_kb_id,
                    "message": "Knowledge base linked successfully"
                }
            else:
                return self._error_response(500, "Failed to create KB link")

        except Exception as e:
            self.logger.error(f"❌ Failed to link knowledge bases: {str(e)}")
            return self._error_response(500, str(e))
    ...
    async def unlink_knowledge_base(
        ...
        """Unlink a knowledge base from this KB"""
        try:
            self.logger.info(f"🔓 Unlinking KB {linked_kb_id} from KB {kb_id}")
            ...
            # Check user permissions on source KB
            user_role = await self.arango_service.get_user_kb_permission(kb_id, user_key)
            if user_role not in ["OWNER", "WRITER", "ORGANIZER"]:
                return self._error_response(403, "User lacks permission to unlink knowledge bases")

            # Remove the link
            result = await self.arango_service.delete_kb_link(kb_id, linked_kb_id)

            if result:
                self.logger.info(f"✅ Successfully unlinked KB {linked_kb_id} from KB {kb_id}")
                return {
                    "success": True,
                    "code": 200,
                ...
                return self._error_response(404, "KB link not found")

        except Exception as e:
            self.logger.error(f"❌ Failed to unlink knowledge bases: {str(e)}")
            return self._error_response(500, str(e))
    ...
    async def get_linked_knowledge_bases(
        ...
        """Get all knowledge bases linked to this KB"""
        try:
            ...
            user_role = await self.arango_service.get_user_kb_permission(kb_id, user_key)
            if not user_role:
                return self._error_response(403, "User has no access to this knowledge base")

            # Get linked KBs
            linked_kbs = await self.arango_service.get_linked_knowledge_bases(kb_id)

            self.logger.info(f"✅ Found {len(linked_kbs)} linked KBs for KB {kb_id}")
            return {
                "success": True,
                "linkedKBs": linked_kbs
            }

        except Exception as e:
            self.logger.error(f"❌ Failed to get linked knowledge bases: {str(e)}")
            return self._error_response(500, str(e))
    ...
    async def get_kb_contributors(
        ...
        """Get contributors (users who have uploaded files) for a KB"""
        try:
            ...
            # Get contributors
            contributors = await self.arango_service.get_kb_contributors(kb_id)
