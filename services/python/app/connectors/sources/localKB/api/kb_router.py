...
@kb_router.post(
    "/{kb_id}/links",
    responses={
        201: {"description": "KB linked successfully"},
        400: {"model": ErrorResponse},
        403: {"model": ErrorResponse},
...
async def link_knowledge_base(
    ...
    try:
        ...
        linked_kb_id = body.get("linkedKbId")
        if not linked_kb_id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="linkedKbId is required"
            )

        user_id = request.state.user.get("userId")
        result = await kb_service.link_knowledge_base(
            kb_id=kb_id,
...
@kb_router.delete(
    "/{kb_id}/links/{linked_kb_id}",
    responses={
        200: {"description": "KB unlinked successfully"},
        403: {"model": ErrorResponse},
        404: {"model": ErrorResponse}
    }
)
...
async def unlink_knowledge_base(
    ...
    try:
        user_id = request.state.user.get("userId")
        result = await kb_service.unlink_knowledge_base(
            kb_id=kb_id,
            linked_kb_id=linked_kb_id,
            user_id=user_id,
        )
...
@kb_router.get(
    "/{kb_id}/links",
    responses={
        200: {"description": "List of linked knowledge bases"},
        403: {"model": ErrorResponse}
    }
)
@inject
async def get_linked_knowledge_bases(
    ...
    try:
        user_id = request.state.user.get("userId")
        result = await kb_service.get_linked_knowledge_bases(
            kb_id=kb_id,
            user_id=user_id,
        )
...
@kb_router.get(
    "/{kb_id}/contributors",
    responses={
        200: {"description": "List of KB contributors"},
        403: {"model": ErrorResponse}
    }
)
@inject
async def get_kb_contributors(
    ...
    try:
        user_id = request.state.user.get("userId")
        result = await kb_service.get_kb_contributors(
            kb_id=kb_id,
            user_id=user_id,
        )
