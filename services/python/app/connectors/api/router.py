...
async def check_beta_connector_access(
    ...
    try:
        ...
        from libs.core.featureflag import FeatureFlags as CONFIG
...
async def get_oauth_authorization_url(
    ...
    try:
        ...
        # Slack strictly requires HTTPS for redirect URIs, except for localhost
        if app_name.upper() == 'SLACK' and redirect_uri.startswith('http://'):
...
@router.delete("/api/v1/connectors/{app_name}")
async def delete_connector(
    ...
    - All records for this connector in ArangoDB
    - All vectors in Qdrant for this connector's records
    - Record groups, edges, and relations
    - The app configuration (reset to unconfigured state)
    - etcd configuration for this connector

    Args:
        app_name: Name of the connector to delete
        request: FastAPI request object
        arango_service: Injected ArangoService dependency

    Returns:
        Dict containing success status and deletion statistics

    Raises:
        HTTPException: 404 if connector not found, 500 for internal errors
    """
    container = request.app.container
    logger = container.logger()
    connector_registry = request.app.state.connector_registry
    ...
    logger.info(f"🗑️ Starting deletion of connector {app_name} for org {user_info['orgId']}")
    ...
    try:
        ...
        # Step 1: Get all records for this connector to get their virtualRecordIds
        logger.info(f"🔍 Step 1: Getting records for connector {app_name}...")
        connector_records = await arango_service.get_connector_records(app_name, org_id)
        virtual_record_ids = list(set([
            r.get("virtualRecordId") for r in connector_records
            if r.get("virtualRecordId")
        ]))

        logger.info(f"📊 Found {len(connector_records)} records with {len(virtual_record_ids)} unique virtualRecordIds")

        # Step 2: Delete vectors from Qdrant
        qdrant_deleted = 0
        if virtual_record_ids:
            logger.info(f"🔍 Step 2: Deleting vectors from Qdrant...")
            try:
                vector_db_service = await VectorDBFactory.create_vector_db_service(
                    "qdrant", config_service, is_async=False
                )
                collection_name = "enterprise_search"

                # Delete vectors in batches by virtualRecordId
                for vid in virtual_record_ids:
                    try:
                        filter_dict = await vector_db_service.filter_collection(
                            must={"virtualRecordId": vid}
                        )
                        vector_db_service.delete_points(collection_name, filter_dict)
                        qdrant_deleted += 1
                    except Exception as e:
                        logger.warning(f"⚠️ Could not delete vectors for virtualRecordId {vid}: {str(e)}")

                logger.info(f"✅ Deleted vectors for {qdrant_deleted} virtualRecordIds from Qdrant")
            except Exception as e:
                logger.warning(f"⚠️ Could not connect to Qdrant or delete vectors: {str(e)}")
        else:
            logger.info("⏭️ Step 2: No vectors to delete (no virtualRecordIds found)")

        # Step 3: Delete all ArangoDB data for this connector
        logger.info(f"🔍 Step 3: Deleting ArangoDB data for connector {app_name}...")
        deletion_stats = await arango_service.delete_connector_data(app_name, org_id)

        # Step 4: Delete etcd configuration for this connector
        logger.info(f"🔍 Step 4: Deleting etcd configuration for connector {app_name}...")
        try:
            filtered_app_name = _sanitize_app_name(app_name)
            config_key = f"/services/connectors/{filtered_app_name}/config"
            # Set config to empty dict to clear it
            await config_service.set_config(config_key, {})
            logger.info(f"✅ Cleared etcd configuration for {app_name}")
        except Exception as e:
            logger.warning(f"⚠️ Could not clear etcd config for {app_name}: {str(e)}")

        # Build response
        response = {
            "success": deletion_stats.get("success", True),
            "message": f"Connector {app_name} deleted successfully",
            "statistics": {
                "records_deleted": deletion_stats.get("records_deleted", 0),
                "record_groups_deleted": deletion_stats.get("record_groups_deleted", 0),
                "edges_deleted": deletion_stats.get("edges_deleted", 0),
                "vectors_deleted": qdrant_deleted,
                "app_reset": deletion_stats.get("app_deleted", False),
            }
        }

        logger.info(f"✅ Connector {app_name} deletion completed: {response['statistics']}")
        return response

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"❌ Failed to delete connector {app_name}: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to delete connector {app_name}: {str(e)}")
