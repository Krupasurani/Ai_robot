...
from libs.core.constants import DefaultEndpoint as DefaultEndpoints
...
NODE_COLLECTIONS = [
    ...
    (CollectionNames.PARENT_CHUNKS.value, None),
]
...
class BaseArangoService:
    ...
    async def _initialize_new_collections(self) -> None:
        ...
        This method is designed to be **idempotent** and **safe under concurrent
        startup** of multiple services. ArangoDB error 1207 (duplicate name)
        can occur when two services attempt to create the same collection at
        nearly the same time. In that case we simply log and continue, using
            ...
            for collection_name, schema in NODE_COLLECTIONS + EDGE_COLLECTIONS:
                ...
                try:
                    ...
                    if self.db.has_collection(collection_name):
                        ...
                        # Slow path: attempt to create the collection.
                        # If another service created it in the meantime,
                        # ArangoDB will raise a 409/1207 duplicate name error.
                    ...
                    if "1207" in error_msg or "duplicate" in error_msg.lower():
                        # Another process created the collection in parallel.
                        # Treat this as success and use the existing collection.
                        self.logger.info(
                            f"✅ Collection '{collection_name}' already exists, using existing collection"
                        )
                        collection = self.db.collection(collection_name)
                    else:
                        # Any other error is unexpected and should fail startup.
                        self.logger.error(
                            f"❌ Failed to create or access collection '{collection_name}': {error_msg}"
                        )
                        raise
    ...
    async def _create_reindex_event_payload(self, record: Dict, file_record: Optional[Dict], user_id: Optional[str] = None, request: Optional[Request] = None) -> Dict:
        ...
        try:
            ...
            if record.get("origin") == OriginTypes.UPLOAD.value:
                storage_url = endpoints.get("storage").get("endpoint", DefaultEndpoints.STORAGE.value)
                signed_url_route = f"{storage_url}/api/v1/document/internal/{record['externalRecordId']}/download"
            else:
                connector_url = endpoints.get("connectors").get("endpoint", DefaultEndpoints.CONNECTOR.value)
        ...
        for field in relevant_fields:
            if field in new:
                if field == "permissionDetails":
                    # Use orjson for faster serialization; OPT_SORT_KEYS ensures deterministic comparison
                    if orjson.dumps(new[field], option=orjson.OPT_SORT_KEYS) != orjson.dumps(
                        existing.get(field, {}), option=orjson.OPT_SORT_KEYS
                    ):
    ...
    async def check_kb_name_exists(
        self,
        user_id: str,
        org_id: str,
        name: str,
        ...
        """Check if a knowledge base with the given name exists for the user (case-insensitive)"""
        try:
            db = transaction if transaction else self.db
            ...
            FOR perm IN @@permissions_collection
                ...
                FILTER LOWER(kb.groupName) == LOWER(@name)
                LIMIT 1
                RETURN 1
            """
            ...
            self.logger.error(f"❌ Failed to check KB name existence: {str(e)}")
            raise
    ...
    async def get_knowledge_base(
        ...
        try:
            ...
            FOR kb IN @@recordGroups_collection
                ...
                LET owner = FIRST(
                    FOR u IN @@users_collection
                        FILTER u._key == kb.createdBy
                        RETURN { fullName: u.fullName, firstName: u.firstName, lastName: u.lastName, photoBase64: u.photoBase64, photoMimeType: u.photoMimeType }
                )
                LET owner_name = (owner.fullName != null && owner.fullName != "") ? owner.fullName : (
                    (owner.firstName != null && owner.firstName != "" && owner.lastName != null && owner.lastName != "") ? CONCAT(owner.firstName, " ", owner.lastName) :
                    ((owner.firstName != null && owner.firstName != "") ? owner.firstName : "Unknown")
                )
                // Get linked KBs (both outgoing and incoming links)
                LET linkedKBs = (
                    LET outgoing = (
                        FOR edge IN @@record_relations_collection
                            FILTER edge._from == kb._id
                            FILTER edge.relationType == "KB_LINK"
                            LET linked_kb = DOCUMENT(edge._to)
                            FILTER linked_kb != null
                            RETURN {
                                id: linked_kb._key,
                                name: linked_kb.groupName,
                                icon: linked_kb.icon,
                                description: linked_kb.description,
                                direction: "outgoing"
                            }
                    )
                    LET incoming = (
                        FOR edge IN @@record_relations_collection
                            FILTER edge._to == kb._id
                            FILTER edge.relationType == "KB_LINK"
                            LET linked_kb = DOCUMENT(edge._from)
                            FILTER linked_kb != null
                            RETURN {
                                id: linked_kb._key,
                                name: linked_kb.groupName,
                ...
                RETURN {
                    ...
                    ownerPhotoBase64: owner.photoBase64,
                    ownerPhotoMimeType: owner.photoMimeType,
                    userRole: user_role,
                    folders: folders,
                    linkedKBs: linkedKBs || []
                }
            ...
            cursor = db.aql.execute(query, bind_vars={
                ...
                "@record_relations_collection": CollectionNames.RECORD_RELATIONS.value,
            })
            result = next(cursor, None)
            if result:
    ...
    async def list_user_knowledge_bases(
        ...
        try:
            ...
            FOR perm IN @@permissions_collection
                ...
                FILTER kb.connectorName == @kb_connector
                {additional_filters}
                // Count documents in this KB (via BELONGS_TO edges)
                LET doc_count = FIRST(
                    FOR belongsEdge IN @@belongs_to_kb
                        FILTER belongsEdge._to == kb._id
                        LET rec = DOCUMENT(belongsEdge._from)
                        FILTER rec != null
                        FILTER rec.isDeleted != true
                        FILTER rec.isFile != false
                ...
                LET ownerName = (owner.fullName != null && owner.fullName != "") ? owner.fullName : (
                    (owner.firstName != null && owner.firstName != "" && owner.lastName != null && owner.lastName != "") ? CONCAT(owner.firstName, " ", owner.lastName) :
                    ((owner.firstName != null && owner.firstName != "") ? owner.firstName : "Unknown")
                )
                ...
                RETURN {{
                    ...
                    icon: kb.icon,
                    description: kb.description,
                    metrics: MERGE(kb.metrics ? kb.metrics : {{}}, {{ docs: doc_count }}),
                    createdAtTimestamp: kb.createdAtTimestamp,
                    updatedAtTimestamp: kb.updatedAtTimestamp,
                    createdBy: kb.createdBy,
                    ownerName: ownerName,
    ...
    async def get_accessible_records(
        ...
        try:
            ...
            LET directAndGroupRecords = UNION_DISTINCT(directRecords, groupRecords, orgRecords, directRecordsPermissionEdge, groupRecordsPermissionEdge)
    ...
    async def get_record_permissions(
        self, record_id: str, org_id: str, transaction: Optional[TransactionDatabase] = None
    ) -> List[str]:
        """
        Get all principals (users, groups, organizations) that have access to a record.
        Used for ACL Push-Down: embedding permissions directly into vector payloads.

        This method traverses the permission graph to find all entities with access:
        - Direct user permissions (via 'permissions' or 'permission' edges)
        - Group permissions (groups that have access to the record)
        - Organization-wide access
        - "Anyone" access (public documents within the org)
        - Inherited permissions from Knowledge Base (via 'belongsTo' edge to recordGroups)

        Args:
            record_id (str): The record ID (_key) to get permissions for
            org_id (str): The organization ID for context
            transaction (Optional[TransactionDatabase]): Optional database transaction

        Returns:
            List[str]: List of principal tokens in format:
                       ["user:<userId>", "group:<groupId>", "org:<orgId>", "anyone:<orgId>"]
        """
        try:
            self.logger.debug("🔐 Getting permissions for record %s in org %s", record_id, org_id)

            # Build AQL query to traverse permission edges and collect all principals
            # We check both 'permissions' and 'permission' edge collections
            # Also check inherited permissions from Knowledge Base
            query = f"""
            LET recordDoc = DOCUMENT("{CollectionNames.RECORDS.value}", @recordId)

            // Direct user permissions via 'permissions' edge
            LET directUserPermissions = (
                FOR principal, edge IN 1..1 ANY recordDoc._id {CollectionNames.PERMISSIONS.value}
                    FILTER edge.type == 'USER' OR IS_SAME_COLLECTION("{CollectionNames.USERS.value}", principal)
                    LET userId = principal.userId != null ? principal.userId : principal._key
                    RETURN DISTINCT CONCAT("user:", userId)
            )

            // Direct user permissions via 'permission' edge (newer schema)
            LET directUserPermissions2 = (
                FOR principal, edge IN 1..1 ANY recordDoc._id {CollectionNames.PERMISSION.value}
                    FILTER edge.type == 'USER' OR IS_SAME_COLLECTION("{CollectionNames.USERS.value}", principal)
                    LET userId = principal.userId != null ? principal.userId : principal._key
                    RETURN DISTINCT CONCAT("user:", userId)
            )

            // Group permissions via 'permissions' edge
            LET groupPermissions = (
                FOR principal, edge IN 1..1 ANY recordDoc._id {CollectionNames.PERMISSIONS.value}
                    FILTER edge.type == 'GROUP' OR IS_SAME_COLLECTION("{CollectionNames.GROUPS.value}", principal)
                    RETURN DISTINCT CONCAT("group:", principal._key)
            )

            // Group permissions via 'permission' edge
            LET groupPermissions2 = (
                FOR principal, edge IN 1..1 ANY recordDoc._id {CollectionNames.PERMISSION.value}
                    FILTER edge.type == 'GROUP' OR IS_SAME_COLLECTION("{CollectionNames.GROUPS.value}", principal)
                    RETURN DISTINCT CONCAT("group:", principal._key)
            )

            // Organization permissions via 'permissions' edge
            LET orgPermissions = (
                FOR principal, edge IN 1..1 ANY recordDoc._id {CollectionNames.PERMISSIONS.value}
                    FILTER edge.type == 'ORG' OR edge.type == 'DOMAIN' OR IS_SAME_COLLECTION("{CollectionNames.ORGS.value}", principal)
                    RETURN DISTINCT CONCAT("org:", principal._key)
            )

            // Organization permissions via 'permission' edge
            LET orgPermissions2 = (
                FOR principal, edge IN 1..1 ANY recordDoc._id {CollectionNames.PERMISSION.value}
                    FILTER edge.type == 'ORG' OR edge.type == 'DOMAIN' OR IS_SAME_COLLECTION("{CollectionNames.ORGS.value}", principal)
                    RETURN DISTINCT CONCAT("org:", principal._key)
            ...
            // Check if record is in 'anyone' collection (public within org)
            LET anyoneAccess = (
                FOR doc IN {CollectionNames.ANYONE.value}
                    FILTER doc.file_key == @recordId AND doc.organization == @orgId
                    RETURN CONCAT("anyone:", @orgId)
            )

            // ========== INHERITED PERMISSIONS FROM KNOWLEDGE BASE ==========
            // Find the Knowledge Base (recordGroup) this record belongs to via 'belongsTo' edge
            // Then get users who have permissions on the KB via 'permissionsToKB' edge
            // Edge direction: users/USER_KEY -> recordGroups/KB_KEY, so we traverse INBOUND from KB
            LET kbPermissions = (
                FOR kb, edge IN 1..1 OUTBOUND recordDoc._id {CollectionNames.BELONGS_TO.value}
                    FILTER IS_SAME_COLLECTION("{CollectionNames.RECORD_GROUPS.value}", kb)
                    // Get user permissions on the KB via 'permissionsToKB' edge (INBOUND because edge is user->kb)
                    FOR user, permEdge IN 1..1 INBOUND kb._id {CollectionNames.PERMISSIONS_TO_KB.value}
                        FILTER permEdge.type == 'USER' AND IS_SAME_COLLECTION("{CollectionNames.USERS.value}", user)
                        LET userId = user.userId != null ? user.userId : user._key
                        RETURN DISTINCT CONCAT("user:", userId)
            )

            // Get group permissions inherited from KB (groups that have access to the KB)
            LET kbGroupPermissions = (
                FOR kb, edge IN 1..1 OUTBOUND recordDoc._id {CollectionNames.BELONGS_TO.value}
                    FILTER IS_SAME_COLLECTION("{CollectionNames.RECORD_GROUPS.value}", kb)
                    FOR group, permEdge IN 1..1 INBOUND kb._id {CollectionNames.PERMISSIONS_TO_KB.value}
                        FILTER permEdge.type == 'GROUP' AND IS_SAME_COLLECTION("{CollectionNames.GROUPS.value}", group)
                        RETURN DISTINCT CONCAT("group:", group._key)
            )

            // Combine all permissions including inherited ones from KB
            RETURN UNION_DISTINCT(
                directUserPermissions,
                directUserPermissions2,
                groupPermissions,
                groupPermissions2,
                orgPermissions,
                orgPermissions2,
            ...
            # Flatten if nested and filter out empty strings
            principals = [p for p in result if p] if result else []

            self.logger.debug(
                "✅ Found %d principals for record %s: %s",
                len(principals), record_id, principals[:5]  # Log first 5 for brevity
            )
            return principals

        except Exception as e:
            self.logger.error(
                "❌ Failed to get permissions for record %s: %s", record_id, str(e)
            )
            return []

    async def get_user_principals(
        self, user_id: str, transaction: Optional[TransactionDatabase] = None
    ) -> List[str]:
        """
        Get all principals (identities) for a user including their groups and org memberships.
        Used for ACL Push-Down: building Qdrant filter at query time.

        This method returns all identity tokens that should be matched against
        the access_control_list field in vector payloads.
        ...
        Returns:
            List[str]: List of principal tokens the user has:
                       ["user:<userId>", "group:<groupId>", "org:<orgId>", "anyone:<orgId>"]
        """
        try:
            self.logger.debug("🔐 Getting principals for user %s", user_id)
            ...
            LET userPrincipal = userDoc != null ? [CONCAT("user:", @userId)] : []

            // Groups the user belongs to
            LET groupPrincipals = (
                FOR group, edge IN 1..1 ANY userDoc._id {CollectionNames.BELONGS_TO.value}
                    FILTER edge.entityType == 'GROUP' OR IS_SAME_COLLECTION("{CollectionNames.GROUPS.value}", group)
                    RETURN DISTINCT CONCAT("group:", group._key)
            )

            // Groups via 'permission' edge (for newer permission model)
            LET groupPrincipals2 = (
                FOR group, edge IN 1..1 ANY userDoc._id {CollectionNames.PERMISSION.value}
                    FILTER edge.type == 'GROUP'
                    RETURN DISTINCT CONCAT("group:", group._key)
            )
            ...
            LET orgPrincipals = (
                FOR org, edge IN 1..1 ANY userDoc._id {CollectionNames.BELONGS_TO.value}
                    FILTER edge.entityType == 'ORGANIZATION' OR IS_SAME_COLLECTION("{CollectionNames.ORGS.value}", org)
                    RETURN DISTINCT CONCAT("org:", org._key)
            )

            // Get org IDs for "anyone" access tokens
            LET orgIds = (
                FOR org, edge IN 1..1 ANY userDoc._id {CollectionNames.BELONGS_TO.value}
                    FILTER edge.entityType == 'ORGANIZATION' OR IS_SAME_COLLECTION("{CollectionNames.ORGS.value}", org)
                    RETURN org._key
            ...
            // Anyone tokens for each org the user belongs to
            LET anyonePrincipals = (
                FOR orgId IN orgIds
                    RETURN CONCAT("anyone:", orgId)
            )
            ...
            db = transaction if transaction else self.db
            cursor = db.aql.execute(query, bind_vars={"userId": user_id})
            result = next(cursor, [])
            ...
            self.logger.debug(
                "✅ Found %d principals for user %s: %s",
                len(principals), user_id, principals
            )
            return principals

        except Exception as e:
            self.logger.error(
                "❌ Failed to get principals for user %s: %s", user_id, str(e)
            )
            return []
    ...
    async def create_kb_link(
        ...
        """Create an edge linking two knowledge bases"""
        try:
            self.logger.info(f"🔗 Creating KB link: {source_kb_id} -> {target_kb_id}")

            # Create edge in record_relations collection (reusing existing edge collection)
            edge_data = {
                "_from": f"{CollectionNames.RECORD_GROUPS.value}/{source_kb_id}",
                "_to": f"{CollectionNames.RECORD_GROUPS.value}/{target_kb_id}",
                "relationType": "KB_LINK",
                "createdBy": user_key,
                "createdAtTimestamp": timestamp,
                "updatedAtTimestamp": timestamp,
            }

            collection = self.db.collection(CollectionNames.RECORD_RELATIONS.value)
            result = collection.insert(edge_data)

            if result:
                self.logger.info(f"✅ Created KB link edge: {result['_key']}")
                return {"success": True, "edgeKey": result['_key']}
            else:
                return {"success": False}

        except Exception as e:
            self.logger.error(f"❌ Failed to create KB link: {str(e)}")
    ...
    async def delete_kb_link(
        ...
        """Delete a KB link edge"""
        try:
            self.logger.info(f"🔓 Deleting KB link: {source_kb_id} -> {target_kb_id}")
            ...
            cursor = self.db.aql.execute(query, bind_vars={
                "source_id": f"{CollectionNames.RECORD_GROUPS.value}/{source_kb_id}",
                "target_id": f"{CollectionNames.RECORD_GROUPS.value}/{target_kb_id}"
            })
            deleted = list(cursor)

            if deleted:
                self.logger.info(f"✅ Deleted KB link edge: {deleted[0]}")
                return True
            else:
                self.logger.warning("⚠️ No KB link found to delete")
                return False

        except Exception as e:
            self.logger.error(f"❌ Failed to delete KB link: {str(e)}")
            return False

    async def get_kb_link(
        self,
        source_kb_id: str,
        target_kb_id: str
    ) -> Optional[Dict]:
        """Check if a KB link exists"""
        try:
            query = f"""
            FOR edge IN {CollectionNames.RECORD_RELATIONS.value}
                FILTER edge._from == @source_id
            ...
            self.logger.error(f"❌ Failed to check KB link: {str(e)}")
            return None
    ...
    async def get_linked_knowledge_bases(
        ...
        """Get knowledge bases linked FROM this KB (unidirectional - only outgoing links)"""
        try:
            self.logger.info(f"📋 Getting linked KBs for KB {kb_id}")

            query = f"""
            FOR edge IN {CollectionNames.RECORD_RELATIONS.value}
                FILTER edge._from == @kb_full_id
                FILTER edge.relationType == "KB_LINK"
                LET linked_kb = DOCUMENT(edge._to)
                FILTER linked_kb != null

                // Count documents in this linked KB (via BELONGS_TO edges)
                LET doc_count = FIRST(
                    FOR belongsEdge IN @@belongs_to_kb
                        FILTER belongsEdge._to == linked_kb._id
                        LET rec = DOCUMENT(belongsEdge._from)
                        FILTER rec != null
                        FILTER rec.isDeleted != true
                        FILTER rec.isFile != false
            ...
            cursor = self.db.aql.execute(query, bind_vars={
                "kb_full_id": f"{CollectionNames.RECORD_GROUPS.value}/{kb_id}",
                "@belongs_to_kb": CollectionNames.BELONGS_TO.value,
            })
            result = list(cursor)

            self.logger.info(f"✅ Found {len(result)} linked KBs")
            return result

        except Exception as e:
            self.logger.error(f"❌ Failed to get linked KBs: {str(e)}")
            return []

    async def get_kb_contributors(
        self,
        kb_id: str
    ) -> List[Dict]:
        """Get users who have uploaded files to a KB (contributors)"""
        try:
            self.logger.info(f"👥 Getting contributors for KB {kb_id}")

            # Query users who have permission on the KB and have uploaded files
            query = f"""
            LET kb_users = (
                FOR perm IN {CollectionNames.PERMISSION.value}
                    FILTER perm._to == @kb_full_id
                    FILTER perm.type == "USER"
                    ...
                    RETURN DISTINCT {{
                        id: user._key,
                        userId: user.userId,
                        name: user.fullName || CONCAT(user.firstName || "", " ", user.lastName || ""),
            ...
            // Return unique users, prioritizing owners
            FOR user IN kb_users
                SORT user.role == "OWNER" DESC, user.name ASC
                RETURN user
            """
            ...
            contributors = list(cursor)

            self.logger.info(f"✅ Found {len(contributors)} contributors for KB {kb_id}")
            return contributors

        except Exception as e:
            self.logger.error(f"❌ Failed to get KB contributors: {str(e)}")
            return []

    # ==================== CONNECTOR DELETION METHODS ====================

    async def get_connector_records(
        self,
        connector_name: str,
        org_id: str
    ) -> List[Dict]:
        """
        Get all records for a specific connector.

        Args:
            connector_name: Name of the connector (e.g., 'DRIVE', 'GMAIL', 'SLACK')
            org_id: Organization ID
        ...
        try:
            self.logger.info(
                f"🔍 Getting all records for connector {connector_name} in org {org_id}"
            )

            query = f"""
            FOR record IN {CollectionNames.RECORDS.value}
                FILTER UPPER(record.connectorName) == UPPER(@connector_name)
                FILTER record.orgId == @org_id
                RETURN {{
                    _key: record._key,
            ...
            self.logger.info(
                f"✅ Found {len(records)} records for connector {connector_name}"
            )
            return records

        except Exception as e:
            self.logger.error(
                f"❌ Failed to get records for connector {connector_name}: {str(e)}"
            )
            return []
    ...
    async def delete_connector_data(
        ...
        """
        Delete all data for a connector including records, record groups,
        edges, parent chunks, and the app configuration.
        ...
        try:
            self.logger.info(
                f"🗑️ Starting deletion of all data for connector {connector_name} in org {org_id}"
            )

            stats = {
                "records_deleted": 0,
                "record_groups_deleted": 0,
            ...
            # Step 1: Delete all records and their edges for this connector
            self.logger.info("🚀 Step 1: Deleting records and edges...")

            # Define edge collections to clean up
            EDGE_COLLECTIONS = [
                CollectionNames.RECORD_RELATIONS.value,
                CollectionNames.BELONGS_TO.value,
                CollectionNames.BELONGS_TO_DEPARTMENT.value,
            ...
            # Delete edges for all records of this connector
            for edge_collection in EDGE_COLLECTIONS:
                try:
                    edge_query = f"""
                    FOR record IN {CollectionNames.RECORDS.value}
                        FILTER UPPER(record.connectorName) == UPPER(@connector_name)
                        FILTER record.orgId == @org_id
                        LET record_id_full = CONCAT('records/', record._key)
                        FOR edge IN @@edge_collection
                            FILTER edge._from == record_id_full OR edge._to == record_id_full
                            REMOVE edge IN @@edge_collection
                            RETURN 1
                    """
                    cursor = self.db.aql.execute(edge_query, bind_vars={
                        "connector_name": connector_name,
                        "org_id": org_id,
                        "@edge_collection": edge_collection
                    })
                    edge_count = len(list(cursor))
                    stats["edges_deleted"] += edge_count
                    self.logger.info(
                        f"✅ Deleted {edge_count} edges from {edge_collection}"
                    )
                except Exception as e:
                    self.logger.warning(
                        f"⚠️ Could not delete edges from {edge_collection}: {str(e)}"
                    )

            # Step 2: Delete records from type-specific collections (files, mails, etc.)
            self.logger.info("🚀 Step 2: Deleting type-specific records...")
            ...
            for type_collection in type_collections:
                try:
                    ...
                    FOR record IN {CollectionNames.RECORDS.value}
                        FILTER UPPER(record.connectorName) == UPPER(@connector_name)
                        FILTER record.orgId == @org_id
                        FOR type_rec IN @@type_collection
                            FILTER type_rec._key == record._key
                            REMOVE type_rec IN @@type_collection
                            RETURN 1
                    """
                    cursor = self.db.aql.execute(type_delete_query, bind_vars={
                        "connector_name": connector_name,
                        "org_id": org_id,
                    ...
                    if count > 0:
                        self.logger.info(
                            f"✅ Deleted {count} records from {type_collection}"
                        )
                except Exception as e:
                    self.logger.warning(
                        f"⚠️ Could not delete from {type_collection}: {str(e)}"
                    )

            # Step 3: Delete records from main records collection
            self.logger.info("🚀 Step 3: Deleting main records...")

            records_delete_query = f"""
            FOR record IN {CollectionNames.RECORDS.value}
                FILTER UPPER(record.connectorName) == UPPER(@connector_name)
                FILTER record.orgId == @org_id
                REMOVE record IN {CollectionNames.RECORDS.value}
                RETURN 1
            """
            cursor = self.db.aql.execute(records_delete_query, bind_vars={
                "connector_name": connector_name,
                "org_id": org_id
            })
            stats["records_deleted"] = len(list(cursor))
            self.logger.info(
                f"✅ Deleted {stats['records_deleted']} records from records collection"
            )

            # Step 4: Delete record groups for this connector
            self.logger.info("🚀 Step 4: Deleting record groups...")

            # First delete edges related to record groups
            for edge_collection in [CollectionNames.BELONGS_TO.value, CollectionNames.PERMISSION.value]:
                try:
                    rg_edge_query = f"""
                    FOR rg IN {CollectionNames.RECORD_GROUPS.value}
                        FILTER UPPER(rg.connectorName) == UPPER(@connector_name)
                        FILTER rg.orgId == @org_id
                        LET rg_id_full = CONCAT('recordGroups/', rg._key)
                        FOR edge IN @@edge_collection
                            FILTER edge._from == rg_id_full OR edge._to == rg_id_full
                            REMOVE edge IN @@edge_collection
                            RETURN 1
                    """
                    cursor = self.db.aql.execute(rg_edge_query, bind_vars={
                        "connector_name": connector_name,
                        "org_id": org_id,
                        "@edge_collection": edge_collection
                    })
                    ...
                    self.logger.warning(
                        f"⚠️ Could not delete record group edges from {edge_collection}: {str(e)}"
                    )
            ...
            FOR rg IN {CollectionNames.RECORD_GROUPS.value}
                FILTER UPPER(rg.connectorName) == UPPER(@connector_name)
                FILTER rg.orgId == @org_id
                REMOVE rg IN {CollectionNames.RECORD_GROUPS.value}
                RETURN 1
            """
            cursor = self.db.aql.execute(rg_delete_query, bind_vars={
                "connector_name": connector_name,
                "org_id": org_id
            })
            stats["record_groups_deleted"] = len(list(cursor))
            self.logger.info(
                f"✅ Deleted {stats['record_groups_deleted']} record groups"
            )

            # Step 5: Delete parent chunks for this connector
            self.logger.info("🚀 Step 5: Deleting parent chunks...")

            try:
                parent_chunks_query = f"""
                FOR chunk IN {CollectionNames.PARENT_CHUNKS.value}
                    FILTER chunk.orgId == @org_id
                    REMOVE chunk IN {CollectionNames.PARENT_CHUNKS.value}
                    RETURN 1
                """
                # Note: Parent chunks don't have connector name, we delete by org
                # This is a simplification - in production you might want to
                # link parent chunks to records and delete based on that
                cursor = self.db.aql.execute(parent_chunks_query, bind_vars={
                    "org_id": org_id
                })
                # We skip this for now as parent chunks don't have connector info
                self.logger.info("⚠️ Parent chunks deletion skipped (no connector association)")
            except Exception as e:
                self.logger.warning(
                    f"⚠️ Could not delete parent chunks: {str(e)}"
                )

            # Step 6: Delete or reset the app configuration
            self.logger.info("🚀 Step 6: Resetting app configuration...")

            try:
                app = await self.get_app_by_name(connector_name)
                if app:
                    # Reset the app to unconfigured state instead of deleting
                    reset_query = f"""
                    FOR app IN {CollectionNames.APPS.value}
                        FILTER LOWER(SUBSTITUTE(app.name, ' ', '')) == LOWER(SUBSTITUTE(@connector_name, ' ', ''))
                        UPDATE app WITH {{
                            isActive: false,
                            isConfigured: false,
                        ...
                        RETURN NEW
                    """
                    cursor = self.db.aql.execute(reset_query, bind_vars={
                        "connector_name": connector_name,
                        "timestamp": get_epoch_timestamp_in_ms()
                    })
                    result = list(cursor)
                    if result:
                        stats["app_deleted"] = True
                        self.logger.info(f"✅ Reset app configuration for {connector_name}")
            except Exception as e:
                self.logger.warning(
                    f"⚠️ Could not reset app configuration: {str(e)}"
                )

            # Step 7: Delete sync points for this connector
            self.logger.info("🚀 Step 7: Deleting sync points...")

            try:
                sync_points_query = f"""
                FOR sp IN {CollectionNames.SYNC_POINTS.value}
                    FILTER CONTAINS(LOWER(sp._key), LOWER(@connector_name))
                    REMOVE sp IN {CollectionNames.SYNC_POINTS.value}
                    RETURN 1
                """
                cursor = self.db.aql.execute(sync_points_query, bind_vars={
                    "connector_name": connector_name
                })
                sync_deleted = len(list(cursor))
                self.logger.info(f"✅ Deleted {sync_deleted} sync points")
            except Exception as e:
                self.logger.warning(
                    f"⚠️ Could not delete sync points: {str(e)}"
                )

            self.logger.info(
                f"✅ Connector deletion completed: {stats}"
            )
            return stats

        except Exception as e:
            self.logger.error(
                f"❌ Failed to delete connector data for {connector_name}: {str(e)}"
            )
            return {
                "success": False,
                "error": str(e),
