...
class KafkaService:
    ...
    async def publish_event(self, topic: str, event: Dict) -> bool:
        ...
        try:
            ...
            # Convert event to JSON bytes using orjson (Rust-based, ~10x faster)
            # orjson.dumps returns bytes directly, no need to encode
            message_value = orjson.dumps(event)
    ...
    async def send_event_to_kafka(self, event_data) -> bool | None:
        ...
        try:
            ...
            # Convert to JSON bytes using orjson (Rust-based, ~10x faster)
            # orjson.dumps returns bytes directly, no need to encode
            message_value = orjson.dumps(formatted_event)
