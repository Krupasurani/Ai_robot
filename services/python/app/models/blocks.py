...
class CitationMetadata(BaseModel):
    ...
    @classmethod
    def validate_bounding_boxes(cls, v: Optional[List[Point]]) -> Optional[List[Point]]:
        """Validate that the bounding boxes contain exactly 4 points when provided.

        Allow None or an empty list to represent 'no bounding box information'.
        """
        if not v:
            # None or empty list -> no bounding box information available
            return v
