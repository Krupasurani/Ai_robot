...
class PyMuPDFOCRStrategy(OCRStrategy):
    def __init__(self, logger, config, language: str = "deu") -> None:
        self.logger = logger
        ...
        # Load German spacy model if language is German, otherwise English
        spacy_model = "de_core_news_sm" if language == "deu" else "en_core_web_sm"
        try:
            self.nlp = self._create_custom_tokenizer(spacy.load(spacy_model))
        except OSError:
            self.logger.warning(f"⚠️ Spacy model '{spacy_model}' not found, falling back to English")
