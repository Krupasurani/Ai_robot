...
class OCRHandler:
    ...
    def __init__(self, logger, strategy_type: str, **kwargs) -> None:
        ...
        Args:
            strategy_type: Type of OCR strategy (currently only "ocrmypdf" is supported)
            **kwargs: Strategy-specific configuration parameters.
                     For OCRMYPDF strategy:
                     - config: Config service
                     - language: OCR language (default: "deu" for German, supports "eng", "deu", etc.)
        """
        self.logger = logger
    ...
    def _create_strategy(self, strategy_type: str, **kwargs) -> OCRStrategy:
        ...
        if strategy_type == OCRProvider.OCRMYPDF.value:
            ...
            self.logger.error(f"❌ Unsupported OCR strategy: {strategy_type}. Only OCRMYPDF is supported.")
            raise ValueError(f"Unsupported OCR strategy: {strategy_type}. Only OCRMYPDF is supported.")
