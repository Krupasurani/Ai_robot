    ...
    async def _fetch_single_url(self, session: aiohttp.ClientSession, url: str) -> str | None:
        ...
        if url.startswith('data:image/'):
            ...
            if url.startswith('data:image/svg+xml'):
                ...
                try:
                    png_base64 = self.svg_base64_to_png_base64(url)
                    return f"data:image/png;base64,{png_base64}"
                except Exception as exc:
                    self.logger.warning(
                        f"Failed to convert inline SVG data URL to PNG; using original SVG. Error: {exc}"
                    )
                    return url
        ...
        try:
            ...
            async with session.head(url, timeout=aiohttp.ClientTimeout(total=10), allow_redirects=True) as head_response:
                ...
                is_valid = self._is_valid_image_content_type(content_type)
                if not is_valid:
            ...
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10), allow_redirects=True) as response:
                ...
                if not is_valid:
                    self.logger.warning(f"⚠️ Content-type changed or invalid during GET: {get_content_type} from URL: {url[:100]}...")
                    return None
                ...
                if not content:
                    self.logger.warning(f"⚠️ Empty content received from URL: {url}")
                    return None

                base64_encoded = base64.b64encode(content).decode('utf-8')

                # Determine extension from content-type or default to png
                extension = get_content_type.split('/')[-1].split(';')[0].strip() if get_content_type.startswith('image/') else 'png'
                ...
                if 'svg' in extension:
                    ...
                    try:
                        png_base64 = self.svg_base64_to_png_base64(base64_encoded)
                        base64_image = f"data:image/png;base64,{png_base64}"
                    except Exception as exc:
                        self.logger.warning(
                            f"⚠️ Failed to convert SVG from URL {url[:100]}... to PNG; using original SVG. Error: {exc}"
                        )
            ...
            if e.status == HTTPStatus.FORBIDDEN:
                ...
                if 'X-Amz-Expires' in str(e) or 'Expires' in str(e):
                    self.logger.warning(
