...
def _event_payload(step: str, message: Optional[str] = None, **extra: Any) -> Dict[str, Any]:
    payload: Dict[str, Any] = {"timestamp": _now_ts(), "step": step}
    if message:
...
def _emit(writer: StreamWriter, event_type: str, step: str, message: Optional[str] = None, **extra: Any) -> None:
    if event_type == "status":
        extra.setdefault("status", step)
    writer({"event": event_type, "data": _event_payload(step, message, **extra)})
...
async def analyze_query_node(state: ChatState, writer: StreamWriter) -> ChatState:
    ...
    try:
        logger = state["logger"]
        _emit(writer, "status", "analyzing", "🧠 Analyzing your request...")
...
async def conditional_retrieve_node(state: ChatState, writer: StreamWriter) -> ChatState:
    ...
    Flow (Thero-style Small-to-Big with Reranking):
    1. Vector Search: Search children broadly (high recall)
    2. Parent Resolution: Deduplicate and load parents from ArangoDB
    3. Reranking: Cross-encoder scores parents for precision
    4. Selection: Return top_k best parents for LLM
    """
    try:
        logger = state["logger"]
        ...
        _emit(writer, "status", "retrieving", "📚 Gathering knowledge sources...")
        ...
        # For Parent-Child architecture: fetch MORE children for better recall,
        # then let reranker select the best parents for precision
        is_complex = analysis.get("is_complex", False)
        base_limit = state["limit"]

        # Fetch more results for reranking (recall-oriented)
        # The reranker will then filter down to base_limit (precision-oriented)
        retrieval_limit = min(base_limit * 3, 100) if reranker_service else min(base_limit * 2, 100)
        if is_complex:
            retrieval_limit = min(retrieval_limit * 2, 150)

        logger.debug(f"Using retrieval limit: {retrieval_limit} (complex: {is_complex}, reranker: {reranker_service is not None})")

        # Step 1 & 2: Vector Search + Parent Resolution (handled by retrieval_service)
        ...
        logger.info(f"✅ Retrieved {len(search_results)} documents (after parent resolution)")

        # Deduplicate by ID first
        seen_ids = set()
        deduplicated_results = []
        for result in search_results:
            result_id = result.get("metadata", {}).get("_id") or result.get("metadata", {}).get("parent_id")
        ...
        logger.debug(f"After deduplication: {len(deduplicated_results)} documents")

        # Step 3: Reranking (precision-oriented)
        # Only rerank if we have more results than needed and reranker is available
        if reranker_service and len(deduplicated_results) > base_limit:
            _emit(writer, "status", "ranking_results", "⚖️ Ranking relevant information...")
            logger.info(f"⚖️ Reranking {len(deduplicated_results)} documents...")
            ...
            # Step 4: Reorder for "Lost in the Middle" optimization
            # LLMs focus more on start and end of context
            final_results = reranker_service.reorder_for_llm(reranked_results)

            logger.info(
                f"✅ Reranking complete: {len(final_results)} top documents "
                f"(top score: {final_results[0].get('reranker_score', 'N/A') if final_results else 'N/A'})"
            )
        else:
            # No reranking - just take top results by vector score
            final_results = deduplicated_results[:base_limit]
            logger.debug(f"Skipped reranking (results: {len(deduplicated_results)}, limit: {base_limit})")

        state["search_results"] = search_results
        state["final_results"] = final_results

        logger.info(f"📊 Final results: {len(state['final_results'])} documents for LLM")
...
def prepare_agent_prompt_node(state: ChatState, writer: StreamWriter) -> ChatState:
    ...
    try:
        ...
        if is_complex:
            ...
            _emit(writer, "status", "thinking", "🧠 Planning complex workflow...")
...
async def agent_node(state: ChatState, writer: StreamWriter) -> ChatState:
    ...
    try:
        ...
        if iteration_count == 0 and is_complex:
            _emit(writer, "status", "planning", "Creating execution plan...")
            ...
            if len(unique_recent) == 1 and len(recent_tools) >= PING_REPEAT_MIN:
                _emit(
                    ...
                    "status",
                    "adapting",
                    f"⚠️ Avoiding repetition - adapting plan (step {iteration_count + 1})...",
                )
            else:
                _emit(
                    writer,
                    "status",
                    "adapting",
                    f"Adapting plan (step {iteration_count + 1})...",
                )
        else:
            _emit(writer, "status", "thinking", "Processing your request...")
...
async def tool_execution_node(state: ChatState, writer: StreamWriter) -> ChatState:
    ...
    try:
        ...
        _emit(
            ...
            "status",
            "executing",
            f"⚙️ Executing tools (step {iteration + 1})...",
...
async def final_response_node(
    ...
    try:
        ...
        _emit(writer, "status", "finalizing", "Generating final response...")
        ...
        if state.get("error"):
            ...
            _emit(writer, "answer_chunk", "answer_chunk", chunk=error_content)

            # Send complete event with error response
            _emit(writer, "complete", "complete", **error_response)
        ...
        if use_existing_response:
            ...
            _emit(writer, "status", "delivering", "Delivering response...")
            ...
            for i in range(0, len(answer_text), chunk_size):
                ...
                _emit(writer, "answer_chunk", "answer_chunk", chunk=chunk)
            ...
            _emit(writer, "complete", "complete", **completion_data)
        ...
        _emit(writer, "status", "generating", "Generating response...")
        ...
        state["error"] = {"status_code": 400, "detail": str(e)}
        _emit(writer, "error", "agent_error", error=str(e))
        return state
...
async def _stream_structured_response(content, writer, logger) -> None:
    ...
    _emit(writer, "complete", "complete", **content)
...
async def _stream_conversational_response(content, writer, logger) -> None:
    ...
    _emit(writer, "complete", "complete", **complete_data)
...
async def _generate_streaming_response(llm, messages, final_results, writer, logger, state) -> Optional[Dict[str, Any]]:
    ...
    try:
        _emit(writer, "status", "generating", "✍️ Creating response...")
