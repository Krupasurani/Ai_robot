...
# New libs imports
from libs.core import get_epoch_timestamp_in_ms
from libs.core.constants import AccountType, Connectors
from libs.messaging import MessagingFactory
        ...
        for org in orgs:
            ...
            for app in enabled_apps:
                ...
                if app["name"].lower() == Connectors.GOOGLE_MAIL.value.lower():
                    ...
                    # Pass extra arguments for specific connectors
                    kwargs = {}
                    if connector_name == Connectors.SLACK.value.lower():
                        kwargs["key_value_store"] = app_container.key_value_store()
...
async def start_kafka_consumers(app_container: ConnectorAppContainer, websocket_manager: WebSocketManager = None) -> List:
    ...
    try:
        ...
        # 3. Create Notification Consumer (WebSocket broadcasting)
        if websocket_manager:
            logger.info("🚀 Starting Notification Kafka Consumer (WebSocket)...")
            notification_kafka_config = await KafkaUtils.create_notification_kafka_consumer_config(app_container)
            notification_kafka_consumer = MessagingFactory.create_consumer(
                broker_type="kafka",
                logger=logger,
                config=notification_kafka_config
            )
            notification_message_handler = await KafkaUtils.create_notification_message_handler(
                app_container, websocket_manager
            )
            await notification_kafka_consumer.start(notification_message_handler)
            consumers.append(("notification", notification_kafka_consumer))
            logger.info("✅ Notification Kafka consumer started (WebSocket broadcasting enabled)")
        else:
            logger.info("⚠️ WebSocket manager not provided, skipping notification consumer")
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    websocket_manager = WebSocketManager(logger=logger)
    app.state.websocket_manager = websocket_manager
    logger.info("✅ WebSocket Manager initialized")

    # Check if Kafka is enabled (not skipped)
    import os
    skip_kafka = os.getenv("SKIP_KAFKA_HEALTH_CHECK", "").lower() in ("true", "1", "yes")

    if skip_kafka:
        logger.warning("⚠️ Running in standalone mode - Kafka messaging disabled")
        app_container.kafka_consumers = []
    else:
        ...
        # Start all Kafka consumers centrally (including WebSocket notification consumer)
        try:
            consumers = await start_kafka_consumers(app_container, websocket_manager)
...
app.include_router(websocket_router)  # WebSocket endpoint for real-time notifications
