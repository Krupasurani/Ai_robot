...
from app.services.messaging.kafka.handlers.notification_handler import (
...
from libs.messaging.kafka import KafkaConsumerConfig, KafkaProducerConfig
...
class KafkaUtils:
    ...
    @staticmethod
    async def create_notification_kafka_consumer_config(app_container: ConnectorAppContainer) -> KafkaConsumerConfig:
        """Create Kafka configuration for notification events (WebSocket broadcasting)"""
    ...
    @staticmethod
    async def create_notification_message_handler(
        app_container: ConnectorAppContainer,
        websocket_manager: WebSocketManager
    ) -> Callable[[Dict[str, Any]], Awaitable[bool]]:
        """Create a message handler for notification events (WebSocket broadcasting)"""
        logger = app_container.logger()
        ...
        async def handle_notification_message(message: Dict[str, Any]) -> bool:
            """Handle incoming record messages and broadcast to WebSocket clients"""
            try:
                if message is None:
                    logger.warning("Received a None message, likely during shutdown. Skipping.")
                    return True
                ...
                if not event_type:
                    logger.debug("Missing event_type in notification message, skipping")
                    return True

                if not payload:
                    logger.debug("Missing payload in notification message, skipping")
                    return True

                # Only process relevant events for WebSocket notifications
                relevant_events = [
                    "newRecord", "updateRecord", "deleteRecord",
                    "reindexRecord", "statusChange", "permissionUpdate"
                ]
                if event_type not in relevant_events:
                    logger.debug(f"Skipping non-notification event: {event_type}")
                    return True

                logger.debug(f"Processing notification event: {event_type}")
                return await notification_handler.process_event(event_type, payload)

            except Exception as e:
                logger.error(f"Error processing notification message: {str(e)}", exc_info=True)
                return False

        return handle_notification_message
