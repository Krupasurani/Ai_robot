...
class AiConfigEventService(BaseEventService):
    ...
    async def __handle_llm_configured(self, payload: dict) -> bool:
        ...
        try:
            ...
            # Refresh the LLM instances with latest configuration (bypass cache)
            await self.retrieval_service.get_llm_instance(role="internal", use_cache=False)
            await self.retrieval_service.get_llm_instance(role="generation", use_cache=False)
    ...
    async def __handle_embedding_configured(self, payload: dict) -> bool:
        ...
        try:
            ...
            # Refresh the embedding model instance with latest configuration (bypass cache)
