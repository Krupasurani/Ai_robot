...
from libs.scheduler import IScheduler as Scheduler
...
class RecordEventHandler(BaseEventService):
    ...
    async def process_event(self, event_type: str, payload: dict) -> bool:
        ...
        try:
            ...
            # ACL Push-Down: Handle permission update event
            if event_type == EventTypes.PERMISSION_UPDATE.value:
                org_id = payload.get("orgId")
                if not org_id:
                    self.logger.error(f"Missing orgId in permission update payload for record {record_id}")
                    return False

                # Sync permissions to vector store without re-indexing content
                success = await self._handle_permission_update(record_id, org_id, virtual_record_id)
                processing_time = (datetime.now() - start_time).total_seconds()
                self.logger.info(
                    f"{'✅' if success else '❌'} Permission sync for record {record_id} "
    ...
    async def _handle_permission_update(
        ...
        """
        Handle permission update event - sync ACL to vector store.

        ACL Push-Down: This updates the access_control_list field in vector payloads
        without requiring a full re-indexing of the document content.

        Args:
            record_id: The record ID to sync permissions for.
            org_id: The organization ID.
            virtual_record_id: Optional virtual record ID.

        Returns:
            True if sync was successful, False otherwise.
        """
        try:
            from app.services.permission_sync import PermissionSyncService

            # Get the vector DB service and collection name from the indexing pipeline
            vector_db_service = self.event_processor.processor.indexing_pipeline.vector_store.vector_db_service
            collection_name = self.event_processor.processor.indexing_pipeline.vector_store.collection_name

            # Create permission sync service
            permission_sync = PermissionSyncService(
                logger=self.logger,
                arango_service=self.event_processor.arango_service,
                vector_db_service=vector_db_service,
                collection_name=collection_name,
            )

            # Sync the permissions
            success = await permission_sync.sync_record_permissions(
                record_id=record_id,
                org_id=org_id,
            ...
            self.logger.error(f"❌ Failed to handle permission update for record {record_id}: {str(e)}")
            return False
