...
from libs.data.vector import VECTOR_DB_COLLECTION_NAME, FilterValue, IVectorDBService
...
class QdrantService(IVectorDBService):
    ...
    async def create_collection(
        ...
        # Binary Quantization optimization: Store original vectors on disk, quantized in RAM
        if vectors_config is None:
            vectors_config = {
                "dense": VectorParams(
                    size=embedding_size,
                    distance=Distance.COSINE,
                    on_disk=True  # Store original float32 vectors on disk for rescoring
                )
            }
        ...
        # Binary Quantization: ~30-40x RAM reduction vs float32
        # Works well with OpenAI text-embedding-3-large, Cohere, and similar models
        # Rescoring with original vectors ensures 100% precision
