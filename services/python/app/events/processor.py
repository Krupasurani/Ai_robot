...
class Processor:
    ...
    async def process_pdf_with_docling(self, recordName, recordId, pdf_binary, virtual_record_id) -> None | bool:
        """
        Process PDF using hybrid approach: OCRmyPDF first, then Docling for structure.

        This is the "best of both worlds" approach:
        1. OCRmyPDF handles OCR (adds text layer with accurate bounding boxes)
        2. Docling handles structure extraction (headings, tables, paragraphs)
        """
        ...
        try:
            ...
            if block_containers is False or block_containers is None:
                self.logger.error(f"❌ Docling failed to process {recordName}")
                return False
            ...
            pipeline = IndexingPipeline(document_extraction=self.document_extraction, sink_orchestrator=self.sink_orchestrator)
            await pipeline.apply(ctx)
            self.logger.info(f"✅ PDF processing completed for record: {recordName} (OCRmyPDF + Docling)")
            return True
        except Exception as e:
    ...
    async def process_pdf_document(
        ...
        try:
            ...
            # Initialize OCR handler with PyMuPDF strategy
