...
from libs.core.constants import (
    ConfigPath as config_node_constants,
)
...
CELERY_QUEUES = (
    ...
    Queue("deep_research", Exchange("deep_research"), routing_key="deep_research"),
)

# Default queue routing
CELERY_ROUTES = {
    "deepresearch.*": {"queue": "deep_research"},
}

# Don't configure celery here from environment - let configure_app() do it
# This prevents connection attempts before the proper config is loaded from etcd
# The configuration will be set properly by CeleryApp.configure_app()
_broker_url = os.getenv("CELERY_BROKER_URL") or os.getenv("REDIS_URL")
if _broker_url:
    celery.conf.update(
        broker_url=_broker_url,
        result_backend=os.getenv("CELERY_RESULT_BACKEND", _broker_url),
...
# Auto-discover tasks from deepresearch module
celery.autodiscover_tasks(["app.services.deepresearch"])
