...
from app.api.routes.chatbot import title_router
from app.api.routes.health import router as health_router
from app.api.routes.image import router as image_router
from app.api.routes.search import router as search_router
from app.containers.query import QueryAppContainer
from app.health.health import Health
from app.modules.web_search import classifier as web_search_classifier  # noqa: F401
from app.modules.web_search.onnx_router import set_use_onnx_router

# App imports - ArangoConfig must match the one used in app/services/graph_db
...
async def initialize_container(container: QueryAppContainer) -> bool:
    ...
    logger.info("✅ HuggingFace web search classifier loaded at startup")
    ...
    try:
        ...
        # Initialize ONNX router configuration
        try:
            use_onnx = await config_service.get_config(
                "/services/webSearch/useOnnxRouter", default=True
            )
            set_use_onnx_router(use_onnx)
            logger.info(
                f"⚙️  Web Search Router: ONNX {'enabled' if use_onnx else 'disabled'}"
            )
        except Exception as e:
            logger.warning(
                f"Failed to load ONNX router config, using default (true): {e}"
            )
            set_use_onnx_router(True)
...
EXCLUDE_PATHS = [
    "/health",
    "/api/v1/llm-health-check",
    "/api/v1/embedding-health-check",
    "/api/v1/health-check",
]  # Exclude health endpoints from authentication for monitoring and startup checks
...
async def health_check() -> JSONResponse:
    ...
    try:
        ...
        connector_endpoint = endpoints.get("connectors").get("endpoint", DefaultEndpoint.CONNECTOR.value)
