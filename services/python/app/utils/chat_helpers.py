...
def get_message_content(flattened_results: List[Dict[str, Any]], virtual_record_id_to_result: Dict[str, Any], user_data: str, query: str, logger, mode: str = "json") -> str:
    ...
    if mode == "simple":
        # If we have no RAG context, fall back to pure chat prompt without citations
        if not flattened_results:
            template = Template(chat_prompt_no_rag)
            rendered_form = template.render(
                query=query,
        ...
        # Build simple context - just blocks with numbers for RAG mode
        chunks = []
        seen_blocks = set()
