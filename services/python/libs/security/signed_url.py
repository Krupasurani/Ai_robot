"""
Signed URL Utilities - Signed URL generation and validation.

This module provides utilities for creating and validating signed URLs
for secure resource access.
"""

from datetime import datetime, timedelta, timezone
from logging import Logger
from typing import TYPE_CHECKING, Any, Dict, Optional

import jwt
from fastapi import HTTPException
from jose import JWTError
from pydantic import BaseModel, ValidationError

from libs.core.constants import ConfigPath, DefaultEndpoint

if TYPE_CHECKING:
    from libs.core.config import ConfigurationService


class SignedUrlConfig(BaseModel):
    """
    Configuration for signed URL generation.

    Attributes:
        private_key: Secret key for signing tokens.
        expiration_minutes: Token expiration time in minutes.
        algorithm: JWT signing algorithm.
        url_prefix: URL prefix for signed URLs.
    """

    private_key: Optional[str] = None
    expiration_minutes: int = 60
    algorithm: str = "HS256"
    url_prefix: str = "/api/v1/index"

    @classmethod
    async def create(
        cls,
        config_service: "ConfigurationService",
    ) -> "SignedUrlConfig":
        """
        Async factory method to create config using configuration service.

        Args:
            config_service: Configuration service to get JWT secret.

        Returns:
            Configured SignedUrlConfig instance.

        Raises:
            ValueError: If private key is not configured.
        """
        try:
            secret_keys = await config_service.get_config(
                ConfigPath.SECRET_KEYS.value
            )
            private_key = secret_keys.get("scopedJwtSecret")
            if not private_key:
                raise ValueError(
                    "Private key must be provided through configuration"
                )
            return cls(private_key=private_key)
        except Exception:
            raise

    def __init__(self, **data) -> None:
        super().__init__(**data)
        if not self.private_key:
            raise ValueError(
                "Private key must be provided through configuration or environment"
            )


class TokenPayload(BaseModel):
    """
    JWT token payload for signed URLs.

    Attributes:
        record_id: ID of the record being accessed.
        user_id: ID of the user making the request.
        exp: Token expiration time.
        iat: Token issued at time.
        additional_claims: Additional custom claims.
    """

    record_id: str
    user_id: str
    exp: datetime
    iat: datetime
    additional_claims: Dict[str, Any] = {}

    class Config:
        json_encoders = {
            datetime: lambda v: v.timestamp()
        }


class SignedUrlHandler:
    """
    Handler for creating and validating signed URLs.

    Usage:
        config = await SignedUrlConfig.create(config_service)
        handler = SignedUrlHandler(logger, config, config_service)

        # Create signed URL
        url = await handler.get_signed_url(record_id, org_id, user_id)

        # Validate token
        payload = handler.validate_token(token)
    """

    def __init__(
        self,
        logger: Logger,
        config: SignedUrlConfig,
        config_service: "ConfigurationService",
    ) -> None:
        """
        Initialize the signed URL handler.

        Args:
            logger: Logger instance.
            config: Signed URL configuration.
            config_service: Configuration service for endpoint lookups.

        Raises:
            ValueError: If configuration is invalid.
        """
        self.logger = logger
        self.signed_url_config = config
        self.config_service = config_service
        self._validate_config()

    def _validate_config(self) -> None:
        """Validate handler configuration."""
        if not self.signed_url_config.private_key:
            raise ValueError("Private key is required")
        if self.signed_url_config.expiration_minutes <= 0:
            raise ValueError("Expiration minutes must be positive")

    async def get_signed_url(
        self,
        record_id: str,
        org_id: str,
        user_id: str,
        additional_claims: Optional[Dict[str, Any]] = None,
        connector: Optional[str] = None,
    ) -> str:
        """
        Create a signed URL with optional additional claims.

        Args:
            record_id: ID of the record to access.
            org_id: Organization ID.
            user_id: User ID making the request.
            additional_claims: Optional additional JWT claims.
            connector: Optional connector name.

        Returns:
            Signed URL string.

        Raises:
            HTTPException: If URL creation fails.
        """
        try:
            # Use UTC consistently
            now_utc = datetime.now(timezone.utc)
            expiration = now_utc + timedelta(
                minutes=self.signed_url_config.expiration_minutes
            )

            # Get connector endpoint
            endpoints = await self.config_service.get_config(
                ConfigPath.ENDPOINTS.value
            )
            connector_endpoint = (
                endpoints.get("connectors", {})
                .get("endpoint", DefaultEndpoint.CONNECTOR.value)
            )

            self.logger.info(f"user_id: {user_id}")

            payload = TokenPayload(
                record_id=record_id,
                user_id=user_id,
                exp=expiration,
                iat=now_utc,
                additional_claims=additional_claims or {},
            )

            # Convert to dict for encoding
            payload_dict = {
                "record_id": record_id,
                "user_id": user_id,
                "exp": payload.exp.timestamp(),
                "iat": payload.iat.timestamp(),
                "additional_claims": additional_claims or {},
            }

            token = jwt.encode(
                payload_dict,
                self.signed_url_config.private_key,
                algorithm=self.signed_url_config.algorithm,
            )

            self.logger.info(
                "Created signed URL for record %s with connector %s",
                record_id,
                connector,
            )

            return (
                f"{connector_endpoint}{self.signed_url_config.url_prefix}"
                f"/{org_id}/{connector}/record/{record_id}?token={token}"
            )

        except ValidationError as e:
            self.logger.error("Payload validation error: %s", str(e))
            raise HTTPException(status_code=400, detail="Invalid payload data")
        except Exception as e:
            self.logger.error("Error creating signed URL: %s", str(e))
            raise HTTPException(status_code=500, detail="Error creating signed URL")

    def validate_token(
        self,
        token: str,
        required_claims: Optional[Dict[str, Any]] = None,
    ) -> TokenPayload:
        """
        Validate the JWT token and optional required claims.

        Args:
            token: JWT token string.
            required_claims: Optional dict of required claim values.

        Returns:
            Validated TokenPayload.

        Raises:
            HTTPException: If validation fails.
        """
        try:
            self.logger.debug(f"Validating token: {token}")
            payload = jwt.decode(
                token,
                self.signed_url_config.private_key,
                algorithms=[self.signed_url_config.algorithm],
            )
            self.logger.debug(f"Payload: {payload}")

            # Convert timestamps back to datetime (ensure UTC timezone)
            if "exp" in payload:
                payload["exp"] = datetime.fromtimestamp(
                    payload["exp"], tz=timezone.utc
                )
            if "iat" in payload:
                payload["iat"] = datetime.fromtimestamp(
                    payload["iat"], tz=timezone.utc
                )

            token_data = TokenPayload(**payload)
            self.logger.debug(f"Token data: {token_data}")

            # Validate required claims
            if required_claims:
                for key, value in required_claims.items():
                    if token_data.additional_claims.get(key) != value:
                        raise HTTPException(
                            status_code=401,
                            detail=f"Required claim '{key}' is invalid",
                        )

            return token_data

        except JWTError as e:
            self.logger.error("JWT validation error: %s", str(e))
            raise HTTPException(status_code=401, detail="Invalid or expired token")
        except ValidationError as e:
            self.logger.error("Payload validation error: %s", str(e))
            raise HTTPException(status_code=400, detail="Invalid token payload")
        except Exception as e:
            self.logger.error("Unexpected error during token validation: %s", str(e))
            raise HTTPException(status_code=500, detail="Error validating token")

